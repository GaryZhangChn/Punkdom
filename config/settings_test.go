...
func TestWriteSettingsFileFiltersPunkdomDir(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "config.toml")
	in := Settings{OpenAIModel: "abc", PunkdomDir: "/tmp/ignored"}
	if err := WriteSettingsFile(p, in); err != nil {
		t.Fatal(err)
	}
	...
	if strings.Contains(string(data), "punkdom_dir") {
		t.Fatalf("punkdom_dir should not be persisted in editable settings: %s", string(data))
	}
}
...
func TestLoadLayeredAppliesAllLayers(t *testing.T) {
	home := t.TempDir()
	ws := t.TempDir()
	if err := os.MkdirAll(filepath.Join(ws, ".punkdom"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if err := WriteSettingsFile(filepath.Join(ws, ".punkdom", "config.toml"), wsCfg); err != nil {
		t.Fatal(err)
	}
...
func TestLoadLayeredIgnoresPunkdomDirFromEditableLayers(t *testing.T) {
	home := t.TempDir()
	ws := t.TempDir()
	if err := os.WriteFile(filepath.Join(home, "config.toml"), []byte("punkdom_dir = \"/tmp/user\"\nopenai_model = \"user-model\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(ws, ".punkdom"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(ws, ".punkdom", "config.toml"), []byte("punkdom_dir = \"/tmp/ws\"\nopenai_model = \"ws-model\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if layered.User.PunkdomDir != "" || layered.Workspace.PunkdomDir != "" {
		t.Fatalf("punkdom_dir should be filtered from editable layers: user=%q workspace=%q", layered.User.PunkdomDir, layered.Workspace.PunkdomDir)
	}
	if layered.Effective.PunkdomDir != normalizePath(home) {
		t.Fatalf("editable layers should not override startup punkdom_dir: %q", layered.Effective.PunkdomDir)
	}
	if layered.Effective.OpenAIModel != "ws-model" {
...
func TestLoadLayeredIgnoresStartupPortsFromWorkspaceLayer(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(ws, ".punkdom", "config.toml"), []byte("backend_port = 19090\nfrontend_port = 16173\n"), 0o644); err != nil {
		t.Fatal(err)
	}
