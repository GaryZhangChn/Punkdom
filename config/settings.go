...
func Merge(parent, child Settings) Settings {
	...
	if child.PunkdomDir != "" {
		out.PunkdomDir = child.PunkdomDir
	}
...
// UserConfigPath 计算用户级配置路径。punkdomDir 已经过 normalizePath 处理。
func UserConfigPath(punkdomDir string) string {
	if punkdomDir == "" {
		punkdomDir = normalizePath("./.punkdom")
	}
	return filepath.Join(punkdomDir, UserConfigFilename)
}
...
// punkdomDir 为空时使用默认 ./.punkdom（后端运行目录下）。
func LoadLayered(punkdomDir, workspace string) (LayeredSettings, error) {
	return LoadLayeredWithGlobal(punkdomDir, workspace, Settings{})
}

// LoadLayeredWithGlobal 读取用户级 + 工作区级配置，并加入全局启动配置层。
func LoadLayeredWithGlobal(punkdomDir, workspace string, global Settings) (LayeredSettings, error) {
	punkdomDir = normalizePath(punkdomDir)
	user, err := ReadSettingsFile(UserConfigPath(punkdomDir))
	if err != nil {
	...
	def := DefaultSettings()
	def.PunkdomDir = punkdomDir
	if global.PunkdomDir == "" {
		global.PunkdomDir = punkdomDir
	} else {
		global.PunkdomDir = normalizePath(global.PunkdomDir)
	}
...
func sanitizeEditableSettings(s Settings) Settings {
	// punkdom_dir 是启动级定位参数，不能由用户级/工作区级配置反向修改自身位置。
	s.PunkdomDir = ""
