...
import (
	...
	"punkdom/config"
	"punkdom/internal/api"
	"punkdom/internal/app"
	"punkdom/internal/observability"
)
...
func main() {
	...
	} else if os.Getenv("PUNKDOM_WORKSPACE") != "" {
		cfg.Workspace = os.Getenv("PUNKDOM_WORKSPACE")
		cfg.ResumeLastWorkspace = false
	}
...
func shouldAutoPickPort() bool {
	if os.Getenv("PUNKDOM_BACKEND_PORT") != "" {
		return false
	}
	explicit := false
...
func resolveSkillsDir(configured string) string {
	...
	if configured != "" && os.Getenv("PUNKDOM_SKILLS_DIR") != "" {
		return configured
	}
	candidates := []string{
		"./skills",
