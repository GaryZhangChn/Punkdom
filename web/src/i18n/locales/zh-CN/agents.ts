const agents = {
  ...
  'agents.field.recentTurns': '兜底最近回合数',
  ...
  'agents.prompt.protectedNote': '自定义提示在行为、创作偏好、策略和风格上优先于 Punkdom 内置提示，但不会覆盖工具权限、输出协议、互动禁写文件、结构化 JSON 要求和后端校验边界。',
  'agents.prompt.builtinNote': '当前展示的是 Punkdom 内置默认系统提示；直接编辑即可在当前层保存为自定义 System Prompt。',
  ...
  'agents.context.compactionValue': '{{threshold}}% 自动压缩，保留 {{count}} 回合；兜底 {{fallback}} 回合',
  'agents.context.recentTurnsNote': '模型配置里的上下文上限 Token 默认 400K；上下文使用率达到阈值时会同步压缩旧历史，并在模型上下文中保留压缩摘要和近期原文回合。最近回合数仅在未配置模型上下文上限或关闭压缩时作为兜底窗口。',
  'agents.context.compactionTargetNote': '这里配置独立上下文压缩 Agent 的输出策略：压缩后保留多少个最近原文回合，以及摘要目标比例。默认保留 8 回合、压到 5%-20%；信息密度高时会倾向上限，避免把关键用户意图、剧情变化和待办压得过短。',
  ...
  'agents.context.compactionTargetValue': '保留 {{count}} 回合，目标 {{min}}%-{{max}}%',
