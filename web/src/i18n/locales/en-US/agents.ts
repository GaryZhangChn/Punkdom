const agents = {
  ...
  'agents.field.recentTurns': 'Fallback Recent Turns',
  ...
  'agents.prompt.protectedNote': 'Custom prompts take priority over Punkdom built-in prompts for behavior, creative preferences, strategy, and style, but they cannot override tool permissions, output protocols, interactive no-write boundaries, structured JSON requirements, or backend validation.',
  'agents.prompt.builtinNote': 'Showing Punkdom’s built-in default system prompt. Edit it to save a custom System Prompt in the current layer.',
  ...
  'agents.context.compactionValue': '{{threshold}}% auto compaction, keep {{count}} turns; fallback {{fallback}} turns',
  'agents.context.recentTurnsNote': 'Model profile context window tokens default to 400K. When context usage reaches the threshold, Punkdom synchronously compacts older history and keeps the compaction summary plus recent raw turns in model context. Recent turns are only the fallback window when no model context window is configured or compaction is disabled.',
  'agents.context.compactionTargetNote': 'Configure the output policy for the independent Context Compaction Agent: how many recent raw turns to keep after compaction and the summary target ratio. Default keeps 8 turns and compresses to 5%-20%; dense context should use the upper end so key user intent, story changes, and pending work are not over-compressed.',
