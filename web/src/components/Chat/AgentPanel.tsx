  ...
  return (
    <aside className="punkdom-sidebar flex h-full min-h-0 flex-col">
      <div className="flex h-10 shrink-0 items-center gap-2 border-b border-[var(--punkdom-border)] px-3">
        ...
        <div className="flex h-7 min-w-0 shrink-0 items-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-0.5" aria-label={t('chat.panelSwitch')}>
          <button
            type="button"
            onClick={() => setView('chat')}
            className={`rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${view === 'chat' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
          >
            {t('chat.view.chat')}
          </button>
          <button
            ...
            className={`rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${view === 'sessions' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1 rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${view === 'review' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
          ...
          <button
            ...
            className={`rounded-[6px] px-1.5 py-0.5 text-[11px] transition-colors ${view === 'traces' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
        ...
        <span className="shrink-0 text-[11px] text-[var(--punkdom-text-faint)]">{isStreaming ? t('chat.status.streaming') : t('chat.status.idle')}</span>
        <button
          type="button"
          onClick={onClose}
      ...
      {view === 'chat' ? (
        <>
          <div className="flex min-h-[42px] shrink-0 items-center gap-2 border-b border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3">
            <IdeTellerSelector workspace={workspace} tellers={tellers} />
            <div className="flex min-w-0 flex-1 items-center text-[11px] text-[var(--punkdom-text-faint)]" title={activeSession ? `${activeSession.title} · ${t('common.messages', { count: activeSession.message_count })}` : t('chat.noSession')}>
              <span className="shrink-0 text-[var(--punkdom-text-muted)]">{t('chat.current')}</span>
            ...
            <button
              ...
              className="punkdom-nav-item flex h-7 shrink-0 items-center gap-1 rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2 text-[11px] disabled:cursor-not-allowed disabled:opacity-45"
...
function IdeTellerSelector({ workspace, tellers }: { workspace: string; tellers: Teller[] }) {
  ...
  return (
    <label className="flex min-w-[126px] max-w-[170px] shrink-0 items-center gap-1.5 text-[11px] text-[var(--punkdom-text-faint)]" title={t('chat.tellerTitle')}>
...
function AgentQuickActions({
  ...
  return (
    <div className="border-b border-[var(--punkdom-border)] bg-[var(--punkdom-bg)] p-3">
      <div className="mb-2 flex items-center gap-2 text-xs font-medium text-[var(--punkdom-text-muted)]">
        ...
        {actions.map((action) => {
          ...
          return (
            <button
              key={action.label}
              type="button"
              className="punkdom-nav-item flex items-center gap-2 border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3 py-2 text-left text-xs"
              onClick={() => onSend(action.prompt)}
            >
              <Icon className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" />
