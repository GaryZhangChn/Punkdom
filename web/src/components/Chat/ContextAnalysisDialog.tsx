...
export const CONTEXT_ANALYSIS_SIMULATED_MESSAGE = '[Punkdom context analysis probe]'
...
export function ContextAnalysisDialog({ open, loading, error, analysis, onOpenChange, onRemoveCompaction }: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[86vh] max-w-5xl flex-col gap-0 overflow-hidden border-[var(--punkdom-border)] bg-[var(--punkdom-bg)] p-0 text-[var(--punkdom-text)]">
        <DialogHeader className="border-b border-[var(--punkdom-border)] px-4 py-3">
          <DialogTitle className="flex items-center gap-2 text-sm">
            <ScrollText className="h-4 w-4 text-[var(--punkdom-text-muted)]" />
            {t('chat.contextAnalysis.title')}
          </DialogTitle>
          <DialogDescription className="text-xs text-[var(--punkdom-text-faint)]">
            {t('chat.contextAnalysis.description')}
          </DialogDescription>
        </DialogHeader>
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
          {loading ? (
            <div className="flex min-h-44 items-center justify-center gap-2 text-xs text-[var(--punkdom-text-muted)]">
              <Loader2 className="h-4 w-4 animate-spin" />
              {t('chat.contextAnalysis.loading')}
            </div>
          ) : error ? (
            <div className="flex min-h-32 items-center gap-2 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-danger-border)] bg-[var(--punkdom-danger-bg)] px-3 py-2 text-xs text-[var(--punkdom-danger)]">
            ...
            <div className="min-h-32 text-xs text-[var(--punkdom-text-faint)]">{t('chat.contextAnalysis.empty')}</div>
          )}
        </div>
      </DialogContent>
    </Dialog>
...
function ContextUsageSummary({ analysis }: { analysis: ContextAnalysis }) {
  ...
  return (
    <div className="grid gap-2 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-2 text-[11px] sm:grid-cols-5">
      {items.map((item) => (
        <div key={item.label} className="min-w-0">
          <div className="truncate text-[var(--punkdom-text-faint)]">{item.label}</div>
          <div className="mt-0.5 truncate font-medium text-[var(--punkdom-text)]">{item.value}</div>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-xs font-medium text-[var(--punkdom-text)]">{title}</h3>
        <span className="text-[11px] text-[var(--punkdom-text-faint)]">{t('chat.contextAnalysis.partCount', { count: parts.length })}</span>
      </div>
      <div className="space-y-2">
        {parts.length > 0 ? parts.map((part, index) => (
          ...
          <div className="rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 py-2 text-xs text-[var(--punkdom-text-faint)]">
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[11px] font-medium text-[var(--punkdom-text)]">{part.title || part.source}</span>
            <span className="block truncate text-[10px] text-[var(--punkdom-text-faint)]">
              {part.source}
        ...
        {onRemoveCompaction && (
          <button
            ...
            className="punkdom-nav-item inline-flex h-7 shrink-0 items-center gap-1 rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-2 text-[11px] text-[var(--punkdom-text-muted)] disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        <span className="shrink-0 text-[10px] text-[var(--punkdom-text-faint)]">{t('chat.contextAnalysis.partSize', { chars: part.chars, bytes: part.bytes })}</span>
      </div>
      {removeCompactionError && <div className="border-t border-[var(--punkdom-border)] px-3 py-2 text-[11px] text-[var(--punkdom-danger)]">{removeCompactionError}</div>}
      {open && (
        <div className="border-t border-[var(--punkdom-border)] p-3">
          {part.content.trim() ? (
            <pre className="max-h-72 overflow-auto whitespace-pre-wrap break-words text-[11px] leading-5 text-[var(--punkdom-text-muted)]">{part.content}</pre>
          ) : (
            <div className="text-[11px] text-[var(--punkdom-text-faint)]">{t('chat.contextAnalysis.emptyPart')}</div>
          )}
        </div>
      )}
    </div>
...
function isCompactionPart(part: ContextAnalysisPart) {
  return part.source === '上下文压缩' || part.content.includes('[Punkdom Context Compaction]')
}
