    ...
    <div className="punkdom-chat-input-area relative border-t border-[var(--punkdom-border)] p-3">
      ...
      {textSelections.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-1.5">
          {textSelections.map((sel, idx) => (
            <span
              key={idx}
              className="inline-flex max-w-full items-center gap-1 rounded-md bg-[var(--punkdom-success-bg)] px-2 py-0.5 text-xs text-[var(--punkdom-success)]"
            >
              <span className="truncate">
                {sel.fileName}:L{sel.startLine}
                {sel.endLine !== sel.startLine && `-L${sel.endLine}`}
                {' '}
                <span className="text-[var(--punkdom-success-muted)]">
              ...
              {onTextSelectionRemove && (
                <button
                  type="button"
                  className="rounded text-[var(--punkdom-success-muted)] hover:text-[var(--punkdom-text)]"
      ...
      <Popover open={showCommands && filteredCommands.length > 0}>
        ...
        <PopoverContent
          align="start"
          side="top"
          className="punkdom-command-menu mb-2 w-[384px] overflow-hidden rounded-lg border border-[var(--punkdom-border)] p-0 text-[var(--punkdom-text)]"
          ...
          <Command shouldFilter={false} className="bg-transparent">
            <div className="border-b border-[var(--punkdom-border-soft)] px-3 py-2">
              <div className="flex items-center justify-between gap-3">
                ...
                <kbd className="shrink-0 rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--punkdom-text-faint)]">/</kbd>
              </div>
            </div>
                ...
                {filteredCommands.map(({ cmd, description, hint, icon: Icon }, index) => {
                  ...
                  return (
                    <CommandItem
                      ...
                      className={`group min-h-12 cursor-pointer rounded-md border px-2.5 py-2 text-[var(--punkdom-text-muted)] ${
                        active
                          ? 'border-[var(--punkdom-border)] bg-[var(--punkdom-active)] text-[var(--punkdom-text)]'
                          : 'border-transparent hover:border-[var(--punkdom-border)] hover:bg-[var(--punkdom-hover)]'
                      }`}
                    >
                    <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-[var(--punkdom-surface-2)] ${
                      active ? 'border-[var(--punkdom-border)] text-[var(--punkdom-text)]' : 'border-[var(--punkdom-border)] text-[var(--punkdom-text-faint)]'
                    }`}>
                      <Icon className="h-3.5 w-3.5" />
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-2">
                        <span className="font-mono text-xs text-[var(--punkdom-text)]">{cmd}</span>
                        <span className="truncate text-xs text-[var(--punkdom-text-muted)]">{description}</span>
                      </span>
                      <span className="mt-0.5 block text-[11px] text-[var(--punkdom-text-faint)]">{hint}</span>
                    </span>
                    </CommandItem>
                  )
                })}
      ...
      <div className="punkdom-chat-composer flex items-end gap-2 rounded-lg border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-1.5">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              type="button"
              size="icon-sm"
              className="h-9 w-9 shrink-0 rounded-md border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)] disabled:opacity-45"
        ...
        <Textarea
          ...
          disabled={disabled}
          rows={1}
          className="min-h-[40px] flex-1 resize-none border-0 bg-transparent px-2.5 py-2 text-sm leading-6 text-[var(--punkdom-text)] shadow-none placeholder:text-[var(--punkdom-text-faint)] focus-visible:border-transparent focus-visible:ring-0 disabled:opacity-50"
        />
        <Button
          type="button"
          ...
          size="icon-sm"
          className={`h-9 w-9 shrink-0 rounded-md text-[var(--punkdom-text)] shadow-[inset_0_1px_0_rgba(255,255,255,0.12)] ${
            disabled ? 'bg-[var(--punkdom-danger-bg)] hover:bg-[var(--punkdom-danger-bg)]' : 'bg-[var(--punkdom-active)] hover:bg-[var(--punkdom-hover)] disabled:bg-[var(--punkdom-active)]'
          }`}
