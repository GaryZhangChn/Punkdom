...
import { listItem, punkdomEase } from '@/features/motion/motion-tokens'
  ...
  const renderMessage = (msg: ChatMessage, index: number) => {
    ...
    return (
      <motion.div
        ...
        initial="initial"
        animate="animate"
        transition={{ duration: 0.18, ease: punkdomEase }}
      >
        {msg.type === 'clear'
          ...
          {messages.length === 0 && !activityContent && (
            <div className="flex justify-start">
              <div className="px-1 py-2">
                <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-[var(--punkdom-text-muted)]" />
              </div>
            </div>
          )}
        </>
  ...
  return (
    <div className="flex justify-start">
      <div className="w-full">
        ...
        {expanded && (
          <div className="space-y-2 border-l border-[var(--punkdom-border)] px-3 py-2">
            {messages.map((message, index) => (
              message.role === 'thinking'
                ? (
                  <div key={message.id || index} className="text-xs leading-relaxed text-[var(--punkdom-text-muted)] whitespace-pre-wrap">
                    {message.content}
                  </div>
                )
                : (
...
function ContextClearDivider({ createdAt }: { createdAt?: string }) {
  ...
  return (
    <div className="flex items-center gap-3 py-1" role="separator" aria-label={t('chat.contextCleared')}>
      <div className="h-px flex-1 bg-[var(--punkdom-border)]" />
      <div className="rounded-full border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 py-1 text-[11px] text-[var(--punkdom-text-muted)]">
