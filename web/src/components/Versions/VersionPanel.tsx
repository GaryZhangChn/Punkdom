...
/** VersionPanel 展示 Punkdom 原生快照版本状态、历史和恢复操作。 */
export function VersionPanel({ workspace, refreshSignal, visible, onClose }: VersionPanelProps) {
  ...
  return (
    <div className="punkdom-sidebar flex h-full min-h-0 flex-col text-xs text-[var(--punkdom-text-muted)]">
      <div className="punkdom-topbar flex h-9 shrink-0 items-center border-b px-3">
        <span className="font-semibold text-[var(--punkdom-text)]">{t('versions.title')}</span>
        <TooltipIconButton label={t('versions.refresh')} className="ml-auto text-[var(--punkdom-text-faint)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" onClick={refresh} disabled={loading}>
          <RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} />
        </TooltipIconButton>
        <TooltipIconButton label={t('versions.close')} className="text-[var(--punkdom-text-faint)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" onClick={onClose}>
      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="px-3 py-2">
          ...
          <div className="mt-3">
            <div className="mb-1 flex items-center gap-2 text-[11px] font-semibold text-[var(--punkdom-text-muted)]">
              <FileClock className="h-3.5 w-3.5" />
              <span>{t('versions.manualSave')}</span>
            </div>
            <Button type="button" size="sm" className="mt-2 flex w-full items-center justify-center gap-2 border border-[var(--punkdom-border)] bg-[var(--punkdom-active)] font-medium text-[var(--punkdom-text)] hover:bg-[var(--punkdom-hover)] disabled:opacity-45" onClick={createManualVersion} disabled={!canCreate}>
