...
const THEME_STYLES: Record<EditorTheme, { labelKey: string; background: string; color: string; accent: string }> = {
  ide: {
    ...
    background: 'var(--punkdom-editor-ide-bg)',
    color: 'var(--punkdom-editor-ide-color)',
    accent: 'var(--punkdom-editor-ide-accent)',
  },
  paper: {
...
const SAVE_STATUS_META: Record<SaveStatus, { labelKey: string; ariaLabelKey: string; className: string; dotClassName?: string; subtle?: boolean }> = {
  dirty: {
    ...
    className: 'text-[var(--punkdom-text-faint)]',
    dotClassName: 'bg-[var(--punkdom-text-faint)] opacity-60',
    subtle: true,
  },
  'auto-saving': {
    ...
    className: 'text-[var(--punkdom-text-faint)]',
    dotClassName: 'animate-pulse bg-[var(--punkdom-text-muted)] opacity-70',
    subtle: true,
  },
  'auto-saved': {
...
export function MarkdownEditor({
  ...
  const editor = useEditor({
    extensions: [
      ...
      /* 自定义 HardBreak：渲染为 <span class="punkdom-hard-break"><br></span>，
      ...
      Node.create({
        ...
        renderHTML() {
          return ['span', { class: 'punkdom-hard-break' }, ['br']]
        },
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* 编辑器工具栏 */}
      <div className="punkdom-editor-toolbar flex h-9 shrink-0 items-center justify-between gap-3 overflow-hidden border-b px-3">
        <div className="flex min-w-0 items-center gap-2 text-xs text-[var(--punkdom-text-muted)]">
          <BookOpen className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" />
          <span className="truncate font-medium text-[var(--punkdom-text)]">{chapterSummary?.display_title || fileName}</span>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button
            ...
            size="xs"
            variant="ghost"
            className="flex items-center gap-1 text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]"
          >
          ...
          <Popover open={settingsOpen} onOpenChange={setSettingsOpen}>
            ...
            <PopoverContent
              align="end"
              side="bottom"
              className="punkdom-editor-settings-panel w-[340px] overflow-hidden rounded-lg border border-[var(--punkdom-border)] p-0 text-[var(--punkdom-text)]"
            >
              <EditorSettingsPanel
                settings={settings}
      ...
      >
        {searchOpen && (
          <div className="sticky top-0 z-20 ml-auto mb-3 flex w-[360px] items-center gap-1 rounded-lg border border-[var(--punkdom-border)] bg-[var(--punkdom-menu-bg)] p-1 shadow-xl backdrop-blur">
            <Search className="ml-2 h-3.5 w-3.5 text-[var(--punkdom-text-muted)]" />
            <input
              ref={searchInputRef}
              value={searchQuery}
              ...
              className="min-w-0 flex-1 bg-transparent px-1 py-1 text-xs text-[var(--punkdom-text)] outline-none placeholder:text-[var(--punkdom-text-faint)]"
            />
            <span className="w-14 text-center text-[11px] text-[var(--punkdom-text-muted)]">
              {searchMatches.length > 0 ? `${searchIndex + 1}/${searchMatches.length}` : '0/0'}
            </span>
            <TooltipIconButton
              label={t('editor.searchPrev')}
              size="icon-xs"
              className="text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]"
      ...
      <div className="punkdom-editor-statusbar flex h-7 shrink-0 items-center gap-4 border-t px-3 text-[11px] text-[var(--punkdom-text-faint)]">
        ...
        {selectedCharacters > 0 && (
          <span className="text-[var(--punkdom-text-muted)]">{t('editor.selectedWords', { count: formatNumber(selectedCharacters) })}</span>
        )}
      </div>
    </div>
  )
...
function EditorSettingsPanel({
  ...
  return (
    <div>
      <div className="border-b border-[var(--punkdom-border-soft)] px-3 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)]">
              <Palette className="h-3.5 w-3.5" />
            </span>
            <div className="min-w-0">
              <div className="text-xs font-medium text-[var(--punkdom-text)]">{t('editor.settings')}</div>
              <div className="text-[11px] text-[var(--punkdom-text-faint)]">{t('editor.settingsDescription')}</div>
            </div>
          </div>
          <button type="button" className="rounded px-2 py-1 text-xs text-[var(--punkdom-text-faint)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" onClick={onClose}>
            {t('common.close')}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-3 p-3">
        <label className="punkdom-editor-control block rounded-lg border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-3">
          <div className="mb-2 flex items-center justify-between gap-3 text-xs">
            ...
            <span className="rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-2 py-0.5 font-mono text-[11px] text-[var(--punkdom-text)]">{settings.lineHeight.toFixed(1)}</span>
          </div>
          <input
            type="range"
            min="1.4"
        ...
        <div>
          <div className="mb-2 flex items-center justify-between text-xs text-[var(--punkdom-text-muted)]">
            <span className="font-medium">{t('editor.backgroundTheme')}</span>
            <span className="text-[11px] text-[var(--punkdom-text-faint)]">{t('editor.currentTheme', { theme: t(THEME_STYLES[settings.theme].labelKey) })}</span>
          </div>
          <div className="grid gap-2">
            {(Object.keys(THEME_STYLES) as EditorTheme[]).map((theme) => (
              <button
                key={theme}
                type="button"
                className={`punkdom-editor-theme-option flex w-full items-center justify-between rounded-lg border px-2.5 py-2 text-left text-xs ${
                  settings.theme === theme
                    ? 'is-active border-[var(--punkdom-border)] bg-[var(--punkdom-active)] text-[var(--punkdom-text)]'
                    : 'border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:border-[var(--punkdom-border)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
                }`}
                onClick={() => patch({ theme })}
              >
                <span className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="mt-0.5 block text-[11px] text-[var(--punkdom-text-faint)]">{t('editor.themePreview')}</span>
                  </span>
                </span>
                {settings.theme === theme && <Check className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-accent-green)]" />}
              </button>
            ))}
          </div>
        </div>
...
function loadEditorSettings(): EditorSettings {
  try {
    const raw = localStorage.getItem('punkdom.editor.settings')
...
function createSearchDecorations(doc: ProseMirrorNode, searchState: SearchState) {
  ...
  const decorations = matches.map((match, index) =>
    Decoration.inline(match.from, match.to, {
      class: index === currentIndex ? 'punkdom-search-match punkdom-search-current' : 'punkdom-search-match',
    }),
  )
...
function selectSearchMatch(editor: Editor, match: SearchMatch) {
  ...
  requestAnimationFrame(() => {
    const el = editor.view.dom.querySelector('.punkdom-search-current') as HTMLElement | null
...
function SelectionToolbar({ editor, onQuote }: { editor: Editor; onQuote: () => void }) {
  ...
  return (
    <div
      ref={toolbarRef}
      className="absolute z-30 flex items-center gap-1 rounded-md border border-[var(--punkdom-border)] bg-[var(--punkdom-menu-bg)] px-1.5 py-1 shadow-xl backdrop-blur"
      style={{ top: coords.top, left: coords.left }}
    >
      <button
        type="button"
        className="flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-menu-item-hover-bg)] hover:text-[var(--punkdom-text)]"
        onClick={onQuote}
