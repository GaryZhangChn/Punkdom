...
import { punkdomEase, subtlePresence } from '@/features/motion/motion-tokens'
...
export function WorkspaceLayout({
  ...
  return (
    <div data-punkdom-app-shell="true" className="h-screen w-screen overflow-hidden">
      <div className="flex h-full flex-col">
        {topBar}
        <div className="flex min-h-0 flex-1">
          {activityBar}
          <Group
            id="punkdom-workspace-horizontal"
            defaultLayout={readStoredLayout('punkdom-workspace-horizontal')}
            onLayoutChanged={(layout) => storeLayout('punkdom-workspace-horizontal', layout)}
          ...
          >
              ...
              <>
                <Panel id="sidebar" defaultSize="20%" minSize="180px" maxSize="36%" className="min-w-[180px]" disabled={!sidebarVisible} hidden={!sidebarVisible} aria-hidden={!sidebarVisible}>
                  <motion.div
                    ...
                    initial="initial"
                    animate="animate"
                    transition={{ duration: 0.16, ease: punkdomEase }}
                  >
                    {sidebar}
                  </motion.div>
                </Panel>
            ...
            <Panel id="center" minSize="30%" className="min-w-0">
              <Group
                id="punkdom-workspace-main-vertical"
                defaultLayout={readStoredLayout('punkdom-workspace-main-vertical')}
                onLayoutChanged={(layout) => storeLayout('punkdom-workspace-main-vertical', layout)}
...
function WorkspaceResizeHandle({ direction, label }: { direction: 'horizontal' | 'vertical'; label: string }) {
  const className = direction === 'vertical'
    ? 'punkdom-resize-handle -mx-1 w-2 cursor-col-resize bg-transparent transition-colors'
    : 'punkdom-resize-handle -my-1 h-2 cursor-row-resize bg-transparent transition-colors'
