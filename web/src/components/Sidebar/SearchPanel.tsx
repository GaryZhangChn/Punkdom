    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0 space-y-2 p-1">
        <div className="relative">
          <Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[var(--punkdom-text-faint)]" />
          <Input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder={t('search.placeholder')}
            className="h-8 border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] pl-8 pr-8 text-xs text-[var(--punkdom-text)] placeholder:text-[var(--punkdom-text-faint)]"
          />
          {loading && (
            <Loader2 className="absolute right-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 animate-spin text-[var(--punkdom-text-faint)]" />
          )}
        </div>
        {trimmedQuery && !loading && results.length > 0 && (
          <div className="px-1 text-[11px] text-[var(--punkdom-text-faint)]">
          ...
          <div className="space-y-3">
            {groups.map((group) => (
              <section key={group.path} className="space-y-1.5">
                <div className="flex min-w-0 items-center gap-1.5 px-1 text-[11px] font-medium text-[var(--punkdom-text-faint)]">
                ...
                <div className="space-y-1">
                  {group.results.map((result, index) => (
                    <button
                      ...
                      type="button"
                      className="punkdom-nav-item block w-full border border-transparent bg-[var(--punkdom-surface)] px-2 py-1.5 text-left hover:border-[var(--punkdom-border)]"
                      onClick={() => void onSelectResult(result, trimmedQuery)}
                    >
                      <div className="mb-1 flex items-center justify-between gap-2 text-[11px] text-[var(--punkdom-text-faint)]">
                      ...
                      <p className="line-clamp-2 whitespace-pre-wrap break-words text-xs leading-5 text-[var(--punkdom-text-muted)]">
  ...
  return (
    <>
      {parts.map((part, index) => part.matched ? (
        <mark key={`${part.text}:${index}`} className="rounded bg-[var(--punkdom-warning-bg)] px-0.5 text-[var(--punkdom-warning)]">
          {part.text}
        </mark>
      ) : (
