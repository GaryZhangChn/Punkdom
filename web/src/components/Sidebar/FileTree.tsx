...
const MENU_CONTENT_CLASS =
  'min-w-[180px] rounded-lg border-[var(--punkdom-border)] bg-[var(--punkdom-menu-bg)] p-1 text-[var(--punkdom-text)] shadow-[0_12px_32px_rgba(0,0,0,0.18)] backdrop-blur'
const MENU_ITEM_CLASS =
  'cursor-pointer rounded-md px-2 py-1.5 text-xs text-[var(--punkdom-text-muted)] transition-colors focus:bg-[var(--punkdom-menu-item-hover-bg)] focus:text-[var(--punkdom-text)] data-[highlighted]:bg-[var(--punkdom-menu-item-hover-bg)] data-[highlighted]:text-[var(--punkdom-text)] [&_svg]:text-[var(--punkdom-tree-icon)] focus:[&_svg]:text-[var(--punkdom-text)] data-[highlighted]:[&_svg]:text-[var(--punkdom-text)]'
const MENU_DANGER_CLASS =
  'text-[var(--punkdom-danger)] focus:bg-[var(--punkdom-danger-bg)] focus:text-[var(--punkdom-danger)] data-[highlighted]:bg-[var(--punkdom-danger-bg)] data-[highlighted]:text-[var(--punkdom-danger)] [&_svg]:text-[var(--punkdom-danger)]'
const MENU_SEPARATOR_CLASS = 'mx-1 my-1 h-px bg-[var(--punkdom-border)]'
      ...
      {showInlineCreate && (
        <li className="flex items-center gap-1 px-2 py-0.5">
          ...
          {inlineEdit!.type === 'create-dir'
            ? <Folder className="h-4 w-4 shrink-0 text-[var(--punkdom-tree-folder)]" />
            : <FileText className="h-4 w-4 shrink-0 text-[var(--punkdom-tree-icon)]" />
          }
          <InlineInput
          ...
          <ContextMenuTrigger asChild>
            <div
              className={`group flex w-full items-center rounded ${
                isMultiSelected
                  ? 'bg-[var(--punkdom-selection-bg)] text-[var(--punkdom-text)]'
                  : isDragOver
                    ? 'bg-[var(--punkdom-drop-bg)] text-[var(--punkdom-text)]'
                    : 'text-[var(--punkdom-tree-text)] hover:bg-[var(--punkdom-hover)]'
              }`}
            ...
            >
              ...
              >
                {expanded ? (
                  <ChevronDown className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-tree-chevron)]" />
                ) : (
                  <ChevronRight className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-tree-chevron)]" />
                )}
                {expanded ? (
                  <FolderOpen className="h-4 w-4 shrink-0 text-[var(--punkdom-tree-folder)]" />
                ) : (
                  <Folder className="h-4 w-4 shrink-0 text-[var(--punkdom-tree-folder)]" />
                )}
                {isRenaming ? (
            ...
            >
              ...
              {isRenaming ? (
                ...
                <span className="flex min-w-0 flex-1 items-center justify-between gap-2">
                  <span className="truncate">{node.name}</span>
                  {chapter && (
                    <span className="flex shrink-0 items-center gap-1 text-[10px] text-[var(--punkdom-text-faint)]">
                      <span>{formatCompactWords(chapter.words)}</span>
                      <span className="rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-1 text-[var(--punkdom-text-muted)]">{chapter.status}</span>
                    </span>
                  )}
                </span>
              )}
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="mr-1 hidden rounded p-0.5 text-[var(--punkdom-tree-icon)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)] group-hover:block"
          onClick={(e) => {
            e.preventDefault()
            e.stopPropagation()
          }}
