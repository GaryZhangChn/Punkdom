...
const fieldCls = 'punkdom-field w-full rounded-[var(--punkdom-radius)] border px-2.5 py-1.5 outline-none placeholder:text-[var(--punkdom-text-faint)] focus:border-[var(--punkdom-field-focus-border)] focus:bg-[var(--punkdom-surface-3)]'
const ghostButtonCls = 'punkdom-nav-item border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] text-[var(--punkdom-text)]'
const defaultSampleChars = 20000
const minSampleChars = 2000
const maxSampleChars = 100000

export function NovelImportDialog({ open, punkdomDir, onOpenChange, onImported }: NovelImportDialogProps) {
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent
          className="punkdom-panel flex max-h-[min(760px,calc(100vh-2rem))] w-[min(620px,calc(100vw-2rem))] max-w-[min(620px,calc(100vw-2rem))] flex-col rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-0 text-[var(--punkdom-text)] shadow-[var(--punkdom-shadow)]"
          aria-describedby="novel-import-desc"
        >
          <div className="border-b border-[var(--punkdom-border)] px-4 py-3">
            <DialogTitle className="text-sm font-semibold text-[var(--punkdom-text)]">{t('novelImport.title')}</DialogTitle>
            <DialogDescription id="novel-import-desc" className="mt-1 text-xs text-[var(--punkdom-text-faint)]">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4 text-xs">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="min-w-0 flex-1 truncate text-[var(--punkdom-text-faint)]">{file ? file.name : t('novelImport.noFile')}</div>
              {previewing && <span className="shrink-0 text-[var(--punkdom-text-muted)]">{previewProgress || t('novelImport.parsing')}</span>}
            </div>

            {preview && (
              <div className="space-y-3 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3 py-3">
                <div className="flex min-w-0 items-center gap-2">
                  <FileText className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" />
                  <div className="min-w-0 flex-1 truncate text-sm font-medium text-[var(--punkdom-text)]">{preview.title}</div>
                </div>
                <div className="flex flex-wrap items-center gap-2 text-[11px] text-[var(--punkdom-text-faint)]">
                ...
                <div className="max-h-36 space-y-1 overflow-y-auto border-t border-[var(--punkdom-border)] pt-2">
                  {preview.chapters.map((chapter) => (
                    <div key={`${chapter.index}-${chapter.title}`} className="flex items-center gap-2 text-[11px] text-[var(--punkdom-text-muted)]">
                      <span className="w-8 shrink-0 text-[var(--punkdom-text-faint)]">{chapter.index}</span>
                      {chapter.volume && <span className="max-w-28 shrink-0 truncate text-[var(--punkdom-text-faint)]">{chapter.volume}</span>}
                      <span className="min-w-0 flex-1 truncate">{chapter.title}</span>
                      <span className="shrink-0 text-[var(--punkdom-text-faint)]">{t('novelImport.chapterChars', { count: chapter.chars.toLocaleString() })}</span>
                    </div>
                  ))}
                  {preview.chapter_count > preview.chapters.length && (
                    <div className="text-[11px] text-[var(--punkdom-text-faint)]">{t('novelImport.moreChapters', { count: preview.chapter_count - preview.chapters.length })}</div>
                  )}
                </div>
            ...
            {preview && (
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  disabled={importing}
                />
                <div className="truncate text-[11px] text-[var(--punkdom-text-faint)]">{t('novelImport.createIn', { dir: punkdomDir || t('importCard.punkdomDir') })}</div>
              </div>
            )}
