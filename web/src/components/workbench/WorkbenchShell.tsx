...
const LEGACY_ACTIVITY_ORDER_STORAGE_KEY = 'punkdom.activity.order.v1'
...
export function WorkbenchShell({
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex h-7 items-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-0.5" aria-label={t('workbench.modeSwitch')}>
          <button
            ...
            className={`relative overflow-hidden rounded-[6px] px-2.5 py-0.5 text-[11px] transition-colors ${navigationMode === 'ide' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
          >
            {navigationMode === 'ide' && <motion.span layoutId="workbench-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--punkdom-active)]" transition={punkdomSpring} />}
            <span className="relative z-10">{t('workbench.mode.ideButton')}</span>
          </button>
          <button
            type="button"
            onClick={() => switchNavigationMode('interactive')}
            className={`relative overflow-hidden rounded-[6px] px-2.5 py-0.5 text-[11px] transition-colors ${navigationMode === 'interactive' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
          >
            {navigationMode === 'interactive' && <motion.span layoutId="workbench-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--punkdom-active)]" transition={punkdomSpring} />}
  ...
  const activityBar = (
    ...
    <aside className={`punkdom-activity-bar flex shrink-0 flex-col gap-2 border-r p-3 transition-[width] duration-500 ease-[var(--punkdom-ease)] ${activityBarExpanded ? 'is-expanded w-48 items-stretch' : 'w-16 items-center'}`}>
      <SortableContext key={activityOrderScope} items={activityItems.map((item) => toSortableActivityId(activityOrderScope, item.id))} strategy={verticalListSortingStrategy}>
        {activityItems.map((item) => (
          <SortableActivityButton
            ...
            active={item.active}
            className="punkdom-icon-button mb-2"
          >
            {item.icon}
          </SortableActivityButton>
        ))}
  ...
  const statusBar = (
    <div className="punkdom-statusbar punkdom-topbar flex h-6 shrink-0 items-center border-t px-3">
      <span>Punkdom v{appVersion}</span>
      {mode === 'ide' && summary && (
          ...
          <LayoutGroup id="workbench-mobile-mode-switch">
            <div className="flex h-8 shrink-0 items-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-0.5" aria-label={t('workbench.modeSwitch')}>
              <button
                type="button"
                onClick={() => switchNavigationMode('ide')}
                className={`relative min-w-0 overflow-hidden rounded-[6px] px-2 py-1 text-[11px] transition-colors ${navigationMode === 'ide' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
              >
                {navigationMode === 'ide' && <motion.span layoutId="workbench-mobile-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--punkdom-active)]" transition={punkdomSpring} />}
                <span className="relative z-10">{t('workbench.mode.ideButton')}</span>
              </button>
              <button
                type="button"
                onClick={() => switchNavigationMode('interactive')}
                className={`relative min-w-0 overflow-hidden rounded-[6px] px-2 py-1 text-[11px] transition-colors ${navigationMode === 'interactive' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
              >
                {navigationMode === 'interactive' && <motion.span layoutId="workbench-mobile-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--punkdom-active)]" transition={punkdomSpring} />}
                <span className="relative z-10">{t('workbench.mode.interactiveButton')}</span>
              </button>
            </div>
          </LayoutGroup>
...
function ActivityIconBadge({ count, children }: { count: number; children: ReactNode }) {
  return (
    <span className="relative inline-flex size-3 items-center justify-center">
      {children}
      {count > 0 && (
        <span className="absolute -right-1.5 -top-1.5 min-w-3 rounded-full bg-[var(--punkdom-danger-border)] px-0.5 text-center text-[8px] leading-3 text-white">
          {count > 9 ? '9+' : count}
        </span>
      )}
    </span>
