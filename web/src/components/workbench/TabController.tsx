...
const TABS_STORAGE_PREFIX = 'punkdom.layout.tabs:'
const ACTIVE_TAB_STORAGE_PREFIX = 'punkdom.layout.activeTab:'
...
export function TabController({
  ...
  return (
    <div className="punkdom-sidebar flex h-9 shrink-0 items-stretch border-b text-xs">
      <LayoutGroup id="editor-tabs">
      <div className="flex min-w-0 flex-1 items-stretch overflow-x-auto">
        {tabs.length === 0 ? (
          <div className="flex h-full items-center px-3 text-[var(--punkdom-text-faint)]">{t('tab.empty')}</div>
        ) : (
            ...
            {tabs.map((tab) => {
              ...
              return (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -8 }}
                  transition={punkdomSpring}
                  className={`group relative flex h-full shrink-0 items-center gap-2 overflow-hidden border-r border-[var(--punkdom-border)] px-3 transition-colors ${
                    isActive
                      ? 'text-[var(--punkdom-text)]'
                      : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)]'
                  }`}
                >
                  {isActive && (
                    <>
                      <motion.span layoutId="editor-tab-active-bg" className="absolute inset-0 bg-[var(--punkdom-active)]" transition={punkdomSpring} />
                      <motion.span layoutId="editor-tab-active-line" className="absolute inset-x-0 top-0 h-0.5 bg-[var(--punkdom-text-faint)]" transition={punkdomSpring} />
                    </>
                  )}
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    className="punkdom-nav-item relative z-10 rounded p-0.5 opacity-0 group-hover:opacity-100"
      ...
      {actions && (
        <div className="flex shrink-0 items-center gap-1 border-l border-[var(--punkdom-border)] px-2">
          {actions}
        </div>
      )}
    </div>
