...
const defaultPayloads: Record<string, unknown> = {
  ...
  '/api/settings': {
    ...
    builtin_agent_prompt_sources: {
      ide: {
        sources: [
          { id: 'runtime_contract', title: '运行契约', source: 'Punkdom runtime', content: '运行契约测试' },
          { id: 'output_protocol', title: '输出格式', source: 'Punkdom runtime', content: '输出格式测试' },
          { id: 'creator', title: 'CREATOR.md', source: 'CREATOR.md', content: '创作者指令测试' },
          { id: 'flow', title: '流程规则', source: 'Punkdom built-in', content: '默认流程测试', editable: true, field: 'flow_prompt' },
          { id: 'custom', title: '用户自定义', source: 'user/workspace config', content: '', editable: true, field: 'system_prompt' },
        ],
      },
      interactive_story: {
        sources: [
          { id: 'runtime_contract', title: '互动运行契约', source: 'Punkdom runtime', content: '互动运行契约测试' },
          { id: 'output_protocol', title: '互动输出格式', source: 'Punkdom runtime', content: '互动输出格式测试' },
          { id: 'flow', title: '流程规则', source: 'Punkdom built-in', content: 'list_interactive_memories read_interactive_memories', editable: true, field: 'flow_prompt' },
          { id: 'custom', title: '用户自定义', source: 'user/workspace config', content: '', editable: true, field: 'system_prompt' },
        ],
      },
    },
    paths: { punkdom_dir: '/punkdom/user', user_config: '', workspace_config: '' },
  },
  '/api/books/create': { workspace: '/punkdom/user/新书', book_meta: { title: '新书', author: '', description: '' } },
...
describe('App', () => {
  ...
  it('creates new books in the Punkdom data directory', async () => {
    const user = userEvent.setup()
    render(
      <TooltipProvider>
        <App />
  ...
  it('opens Agents as a global management page and toggles back', async () => {
    ...
    expect(screen.getByText('当前展示的是 Punkdom 内置默认系统提示；直接编辑即可在当前层保存为自定义 System Prompt。')).toBeInTheDocument()
