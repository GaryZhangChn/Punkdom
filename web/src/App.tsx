...
const PROJECT_VISIBLE_KEY = 'punkdom.layout.projectVisible'
const ACTIVITY_BAR_EXPANDED_KEY = 'punkdom.layout.activityBarExpanded'
const INTERACTIVE_RIGHT_VISIBLE_KEY = 'punkdom.layout.interactiveRightVisible'
...
function App() {
  ...
  useEffect(() => {
    ...
    const reload = () => {
      fetchSettings()
        .then((data) => {
          ...
          setPunkdomDir(data?.paths?.punkdom_dir || '')
    ...
    window.addEventListener('punkdom:settings-updated', onUpdated)
    return () => {
      cancelled = true
      window.removeEventListener('punkdom:settings-updated', onUpdated)
    }
  }, [setTheme, workspace])
  ...
  const handleCharacterCardImport = useCallback(async () => {
    ...
    try {
      ...
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('punkdom:lore-updated', { detail: result }))
      }, 0)
      notifyVersionChange()
  ...
  return (
    <PunkdomMotionProvider intensity={motionIntensity}>
      <ModeRouter
        mode={mode}
