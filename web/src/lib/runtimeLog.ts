const RUNTIME_LOG_KEY = 'punkdom.runtime.logs'
const MAX_LOGS = 80
...
export function recordRuntimeLog(entry: Omit<RuntimeLogEntry, 'url' | 'userAgent' | 'timestamp'>) {
  ...
  console.error('[punkdom-runtime]', fullEntry)
  try {
    ...
    console.error('[punkdom-runtime] 写入本地日志失败', error)
  }
}
...
function detectWhiteScreenReason(root: HTMLElement | null) {
  ...
  if (!root.querySelector('[data-punkdom-app-shell="true"]')) return 'App shell 未挂载或渲染中断'
  return ''
}
