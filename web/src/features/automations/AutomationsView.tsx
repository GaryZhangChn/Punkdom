...
const fieldCls = 'punkdom-field min-h-7 rounded-[var(--punkdom-radius)] border px-2.5 py-1.5 outline-none placeholder:text-[var(--punkdom-text-faint)] focus:border-[var(--punkdom-field-focus-border)] focus:bg-[var(--punkdom-surface-3)]'
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col bg-[var(--punkdom-bg)] text-[var(--punkdom-text)]">
      <div className="punkdom-topbar flex min-h-10 shrink-0 flex-wrap items-center gap-2 border-b px-4 py-1.5 text-xs">
        <Clock3 className="h-3.5 w-3.5 text-[var(--punkdom-text-muted)]" />
        ...
        <div className="ml-3 flex gap-1 border-l border-[var(--punkdom-border)] pl-3">
          {(['workspace', 'user'] as const).map((scope) => (
            <button key={scope} type="button" onClick={() => switchScope(scope)} className={`${tabCls} ${scopeFilter === scope ? 'is-active' : 'bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)]'}`}>
              {scope === 'workspace' ? t('automations.scope.workspace') : t('automations.scope.user')}
            </button>
          ))}
        </div>
        <button type="button" onClick={checkTriggers} disabled={!activeId || running || saving} className="punkdom-nav-item ml-auto inline-flex items-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 py-1 text-[var(--punkdom-text-muted)] disabled:opacity-50">
          <RefreshCw className="h-3.5 w-3.5" />
          {t('automations.checkTriggers')}
        </button>
        <button type="button" onClick={runNow} disabled={!activeId || running || saving} className="punkdom-nav-item inline-flex items-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-active)] px-3 py-1 text-[var(--punkdom-text)] disabled:opacity-50">
          <Play className="h-3.5 w-3.5" />
          {running ? t('automations.running') : t('automations.runNow')}
        </button>
        {running && (
          <button type="button" onClick={runStream.stop} className="punkdom-nav-item inline-flex items-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 py-1 text-[var(--punkdom-text-muted)]">
            <Square className="h-3.5 w-3.5" />
            {t('automations.stopRun')}
          </button>
        )}
        <button type="button" onClick={save} disabled={saving || running} className="punkdom-nav-item inline-flex items-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-active)] px-3 py-1 text-[var(--punkdom-text)] disabled:opacity-50">
          {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
          {t('common.save')}
        </button>
        {onClose && (
          <button type="button" onClick={onClose} className="punkdom-nav-item rounded p-1" aria-label={t('automations.close')} title={t('automations.close')}>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[18rem_minmax(0,1fr)] text-xs">
        <aside className="min-h-0 overflow-y-auto border-r border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-3">
          <div className="mb-3 grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setPanelView('agent')} className={`punkdom-nav-item inline-flex h-8 items-center justify-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] px-2 ${panelView === 'agent' ? 'is-active' : 'bg-[var(--punkdom-surface)]'}`}>
              <Bot className="h-3.5 w-3.5" />
              <span className="min-w-0 truncate">{t('automations.view.agent')}</span>
            </button>
            <button type="button" onClick={() => createNew(scopeFilter)} className="punkdom-nav-item inline-flex h-8 items-center justify-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-active)] px-2">
          ...
          <div className="space-y-1">
            {filteredTasks.length === 0 ? (
              <div className="px-2 py-8 text-center text-[var(--punkdom-text-faint)]">{t('automations.empty')}</div>
            ) : filteredTasks.map((task) => (
              <button key={task.id} type="button" onClick={() => selectTask(task)} className={`punkdom-nav-item flex w-full items-start gap-2 rounded-[var(--punkdom-radius)] px-2.5 py-2 text-left ${activeId === task.id ? 'is-active' : ''}`}>
                <FileText className="mt-0.5 h-4 w-4 shrink-0 text-[var(--punkdom-text-muted)]" />
                  <span className="min-w-0 flex-1">
                  <span className="block truncate font-medium text-[var(--punkdom-text)]">{task.name}</span>
                  <span className="mt-0.5 block truncate text-[11px] text-[var(--punkdom-text-faint)]">{automationTaskSubtitle(task, t)} · {task.enabled ? t('automations.enabled') : t('automations.disabled')}</span>
                </span>
              </button>
            ))}
          </div>
        ...
        <main className="flex min-h-0 flex-col">
          <div className="flex h-10 shrink-0 items-center gap-2 border-b border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-4">
            <div className="flex h-7 items-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-0.5">
              <button
                type="button"
                onClick={() => setPanelView('config')}
                className={`inline-flex items-center gap-1.5 rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${panelView === 'config' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
              >
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${panelView === 'inbox' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
              >
                <Inbox className="h-3.5 w-3.5" />
                {t('automations.view.inbox')}
                {unreadInboxCount > 0 && <span className="rounded-full bg-[var(--punkdom-danger-border)] px-1.5 text-[10px] text-white">{unreadInboxCount}</span>}
              </button>
              <button
                type="button"
                onClick={() => setPanelView('run')}
                className={`inline-flex items-center gap-1.5 rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${panelView === 'run' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
              >
            ...
            {runStream.activeRun && (
              <span className="truncate rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2 py-0.5 font-mono text-[11px] text-[var(--punkdom-text-faint)]">
          ...
          {panelView === 'config' ? (
            <div className="min-h-0 flex-1 overflow-y-auto">
              <div className="mx-auto flex max-w-5xl flex-col gap-5 px-6 py-5">
                <section className="grid gap-3 border-b border-[var(--punkdom-border)] pb-5 md:grid-cols-2">
                ...
                <section className="space-y-3 pb-5">
                  <div className="flex items-center justify-between">
                    ...
                    {activeId && <button type="button" onClick={remove} disabled={saving || running} className="punkdom-nav-item inline-flex items-center gap-1.5 rounded-[var(--punkdom-radius)] px-2 py-1 text-[var(--punkdom-text-muted)] disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" />{t('common.delete')}</button>}
                  </div>
            ...
            <section className="flex min-h-0 flex-1 flex-col">
              ...
              {runStream.activeRun ? (
                ...
                <div className="border-t border-[var(--punkdom-border)] px-4 py-3 text-[11px] text-[var(--punkdom-text-faint)]">
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return <label className="flex flex-col gap-1.5 text-xs"><span className="text-[var(--punkdom-text-muted)]">{label}</span>{children}</label>
}

function SectionTitle({ title }: { title: string }) {
  return <div className="text-xs font-medium text-[var(--punkdom-text)]">{title}</div>
}
...
function RunList({ task, activeRunId, onOpenRun }: { task: AutomationTask; activeRunId: string; onOpenRun: (run: AutomationRunRecord) => void }) {
  ...
  if (runs.length === 0) return <div className="rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3 py-8 text-center text-[var(--punkdom-text-faint)]">{t('automations.runs.empty')}</div>
  return (
    <div className="space-y-2">
      {runs.slice(0, 5).map((run) => (
        <div key={run.id} className="rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3 py-2">
          <div className="flex items-center gap-2">
            <span className="font-medium">{run.status}</span>
            <span className="text-[11px] text-[var(--punkdom-text-faint)]">{new Date(run.started_at).toLocaleString()}</span>
            {run.output_path && <span className="ml-auto truncate text-[11px] text-[var(--punkdom-text-faint)]">{run.output_path}</span>}
            {run.session_id && (
              <button
                type="button"
                onClick={() => onOpenRun(run)}
                className={`punkdom-nav-item ml-auto rounded-[var(--punkdom-radius)] px-2 py-0.5 text-[11px] ${activeRunId === run.id ? 'is-active' : 'text-[var(--punkdom-text-muted)]'}`}
              >
                {t('automations.runs.viewTimeline')}
              </button>
            )}
          </div>
          <div className="mt-1 line-clamp-3 whitespace-pre-wrap text-[11px] leading-5 text-[var(--punkdom-text-muted)]">{run.error || run.summary}</div>
        </div>
      ))}
    </div>
  )
