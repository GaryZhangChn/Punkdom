...
export function SkillsView({ workspace, onClose, onRequestAgent }: SkillsViewProps) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      setNewAgents(['ide'])
      setMode('editor')
      window.dispatchEvent(new CustomEvent('punkdom:skills-updated'))
      await load()
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col bg-[var(--punkdom-bg)] text-[var(--punkdom-text)]">
      <div className="punkdom-topbar flex min-h-10 shrink-0 flex-wrap items-center gap-2 border-b px-4 py-1.5 text-xs">
        <Sparkles className="h-3.5 w-3.5 text-[var(--punkdom-text-muted)]" />
        <span className="font-medium">{t('skills.title')}</span>
        <span className="min-w-0 truncate text-[11px] text-[var(--punkdom-text-faint)]">{t('skills.subtitle')}</span>
        <button
          type="button"
          onClick={() => void load()}
          disabled={loading}
          className="punkdom-nav-item ml-auto inline-flex items-center gap-1.5 rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2.5 py-1 disabled:opacity-50"
        >
        ...
        <button
          ...
          className="punkdom-nav-item inline-flex items-center gap-1.5 rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-active)] px-2.5 py-1 disabled:cursor-not-allowed disabled:opacity-45"
        >
          {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
          {t('common.save')}
        </button>
        {onClose && (
          <button type="button" onClick={onClose} className="punkdom-nav-item rounded p-1" aria-label={t('common.close')} title={t('common.close')}>
      ...
      <div className={`grid min-h-0 flex-1 text-xs ${agentOpen ? 'grid-cols-[20rem_minmax(0,1fr)_minmax(320px,28rem)]' : 'grid-cols-[20rem_minmax(0,1fr)]'}`}>
        <aside className="min-h-0 overflow-y-auto border-r border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-3">
          <div className="mb-4 grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={askAgent}
              className={`punkdom-nav-item inline-flex h-8 items-center justify-center gap-1.5 rounded border border-[var(--punkdom-border)] px-2 ${agentOpen ? 'is-active' : 'bg-[var(--punkdom-surface)]'}`}
            >
            ...
            <button
              ...
              className={`punkdom-nav-item inline-flex h-8 items-center justify-center gap-1.5 rounded border border-[var(--punkdom-border)] px-2 ${mode === 'create' ? 'is-active' : 'bg-[var(--punkdom-surface)]'}`}
            >
        ...
        <main className="flex min-h-0 flex-col">
          {mode === 'create' ? (
            ...
            <>
              <div className="flex min-h-12 shrink-0 items-center gap-3 border-b border-[var(--punkdom-border)] px-4">
                <FileCode2 className="h-4 w-4 text-[var(--punkdom-text-muted)]" />
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sm text-[var(--punkdom-text)]">/{document.name}</span>
                    <span className="rounded bg-[var(--punkdom-surface-2)] px-1.5 py-0.5 text-[10px] text-[var(--punkdom-text-muted)]">{scopeLabel(document.scope, t)}</span>
                    {!document.active && <span className="rounded bg-[var(--punkdom-warning-bg)] px-1.5 py-0.5 text-[10px] text-[var(--punkdom-warning)]">{t('skills.shadowed')}</span>}
                    {document.agent && <span className="rounded bg-[var(--punkdom-surface-2)] px-1.5 py-0.5 text-[10px] text-[var(--punkdom-text-muted)]">{document.agent}</span>}
                    {!document.editable && <Lock className="h-3.5 w-3.5 text-[var(--punkdom-text-faint)]" />}
                  </div>
                  <div className="mt-0.5 truncate text-[11px] text-[var(--punkdom-text-faint)]" title={document.path}>{document.path}</div>
                </div>
                {dirty && <span className="text-[11px] text-[var(--punkdom-warning)]">{t('skills.unsaved')}</span>}
                {document.editable && (
                  <>
                    <button
                      type="button"
                      onClick={openConfig}
                      className="punkdom-nav-item inline-flex h-7 shrink-0 items-center gap-1 rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2 text-[11px]"
                    >
                    ...
                    <button
                      ...
                      disabled={saving}
                      className="punkdom-nav-item inline-flex h-7 shrink-0 items-center gap-1 rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2 text-[11px] text-[var(--punkdom-danger)] disabled:cursor-not-allowed disabled:opacity-45"
                    >
              ...
              <Textarea
                ...
                spellCheck={false}
                className="min-h-0 flex-1 resize-none rounded-none border-0 bg-[var(--punkdom-bg)] px-5 py-4 font-mono text-xs leading-5 text-[var(--punkdom-text)] shadow-none focus-visible:ring-0"
              />
            </>
          ) : (
            <div className="flex h-full items-center justify-center px-6 text-center text-xs text-[var(--punkdom-text-faint)]">
        ...
        <section className="border-b border-[var(--punkdom-border)] pb-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)]">
              <Plus className="h-4 w-4 text-[var(--punkdom-text-muted)]" />
            </div>
            <div className="min-w-0">
              <h1 className="truncate text-sm font-semibold">{t('skills.create.title')}</h1>
              <div className="mt-1 text-[11px] text-[var(--punkdom-text-faint)]">{t('skills.create.subtitle')}</div>
            </div>
          </div>
        </section>

        {scopes.length === 0 ? (
          <div className="rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3 py-3 text-[11px] leading-5 text-[var(--punkdom-text-faint)]">
            ...
            <section className="space-y-3 border-b border-[var(--punkdom-border)] pb-5">
              ...
              <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                  ...
                  <div className="flex gap-1">
                    {scopes.map((item) => (
                      <button
                        ...
                        className={`punkdom-nav-item h-8 flex-1 rounded-[var(--punkdom-radius)] px-2 ${scope === item.scope ? 'is-active' : 'bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)]'}`}
                      >
                ...
                <Field label={t('skills.create.name')}>
                  <input
                    ...
                    className="punkdom-field h-8 w-full rounded-[var(--punkdom-radius)] border px-2.5 font-mono outline-none aria-invalid:border-[var(--punkdom-danger)]"
                  />
                  <div className={`mt-1 text-[11px] ${invalidName ? 'text-[var(--punkdom-danger)]' : 'text-[var(--punkdom-text-faint)]'}`}>
              ...
              <Field label={t('skills.create.description')}>
                <input
                  ...
                  className="punkdom-field h-8 w-full rounded-[var(--punkdom-radius)] border px-2.5 outline-none"
                />
                <div className="mt-1 text-[11px] text-[var(--punkdom-text-faint)]">{t('skills.create.descriptionHint')}</div>
              </Field>
            </section>
            ...
            <section className="space-y-3 pb-5">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="punkdom-nav-item inline-flex h-8 items-center justify-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-active)] px-3 disabled:cursor-not-allowed disabled:opacity-45"
                >
                ...
                <button
                  type="button"
                  onClick={onAskAgent}
                  className="punkdom-nav-item inline-flex h-8 items-center justify-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3"
                >
                  <Bot className="h-3.5 w-3.5" />
    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      <div className="mx-auto flex max-w-5xl flex-col gap-5 px-6 py-5">
        <section className="border-b border-[var(--punkdom-border)] pb-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)]">
              <Settings2 className="h-4 w-4 text-[var(--punkdom-text-muted)]" />
            </div>
            <div className="min-w-0">
              <h1 className="truncate text-sm font-semibold">{t('skills.config.title')}</h1>
              <div className="mt-1 text-[11px] text-[var(--punkdom-text-faint)]">{t('skills.config.subtitle')}</div>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-3 border-b border-[var(--punkdom-border)] pb-5">
          ...
          <Field label={t('skills.create.description')}>
            <input
              ...
              className="punkdom-field h-8 w-full rounded-[var(--punkdom-radius)] border px-2.5 outline-none"
            />
            <div className={`mt-1 text-[11px] ${trimmedDescription ? 'text-[var(--punkdom-text-faint)]' : 'text-[var(--punkdom-danger)]'}`}>
        ...
        <section className="flex flex-wrap gap-2 pb-5">
          ...
          <button
            ...
            onClick={onDelete}
            disabled={saving}
            className="punkdom-nav-item ml-auto inline-flex h-8 items-center justify-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 text-[var(--punkdom-danger)] disabled:cursor-not-allowed disabled:opacity-45"
          >
            <Trash2 className="h-3.5 w-3.5" />
            {t('skills.delete.action')}
          </button>
  ...
  return (
    <div className="space-y-3">
      {agentGroups.map((group) => (
        <div key={group.group}>
          <div className="mb-1.5 text-[11px] font-medium text-[var(--punkdom-text-faint)]">{t(group.group)}</div>
          <div className="grid gap-2 md:grid-cols-2">
            {group.agents.map((agent) => {
              ...
              return (
                <label
                  key={agent.key}
                  className={`punkdom-nav-item flex min-h-14 cursor-pointer items-center gap-3 rounded-[var(--punkdom-radius)] border px-3 py-2 ${checked ? 'is-active border-[var(--punkdom-border)]' : 'border-transparent bg-[var(--punkdom-surface)] text-[var(--punkdom-text-muted)] hover:border-[var(--punkdom-border)]'}`}
                >
                  <input
                    type="checkbox"
                    checked={checked}
  ...
  return (
    <section>
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <div className="font-medium text-[var(--punkdom-text-muted)]">{scopeLabel(scope, t)}</div>
        <div className="text-[10px] text-[var(--punkdom-text-faint)]">{scopeInfo?.writable ? t('skills.scope.editable') : t('skills.scope.readonly')}</div>
      </div>
      {scopeInfo?.path && <div className="mb-2 truncate font-mono text-[10px] text-[var(--punkdom-text-faint)]" title={scopeInfo.path}>{scopeInfo.path}</div>}
      {skills.length === 0 ? (
        <div className="rounded border border-dashed border-[var(--punkdom-border)] px-2 py-3 text-center text-[11px] text-[var(--punkdom-text-faint)]">{t('skills.scope.empty')}</div>
      ) : (
          ...
          {skills.map((skill) => {
            ...
            return (
              <button
                ...
                className={`punkdom-nav-item w-full rounded border px-2.5 py-2 text-left ${
                  active
              ...
              >
                <span className="flex items-center gap-2">
                  <span className="min-w-0 flex-1 truncate font-mono text-xs text-[var(--punkdom-text)]">/{skill.name}</span>
                  {skill.active ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-success)]" /> : <span className="shrink-0 text-[10px] text-[var(--punkdom-warning)]">{t('skills.shadowed')}</span>}
                  {!skill.editable && <Lock className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />}
                </span>
                <span className="mt-1 line-clamp-2 block text-[11px] leading-4 text-[var(--punkdom-text-faint)]">{skill.description}</span>
              </button>
            )
          })}
        </div>
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="block">
      <span className="mb-1.5 block text-[11px] font-medium text-[var(--punkdom-text-muted)]">{label}</span>
      {children}
    </div>
  )
}
...
function PreviewRow({ label, value, wide = false }: { label: string; value: string; wide?: boolean }) {
  return (
    <div className={`rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3 py-2 ${wide ? 'md:col-span-2' : ''}`}>
      <div className="text-[10px] uppercase text-[var(--punkdom-text-faint)]">{label}</div>
      <div className="mt-1 truncate font-mono text-xs text-[var(--punkdom-text)]" title={value}>{value}</div>
    </div>
  )
}
