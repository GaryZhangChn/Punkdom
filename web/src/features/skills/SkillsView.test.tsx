...
const baseSnapshot: SkillSnapshot = {
  scopes: [
    { scope: 'builtin', path: '/punkdom/builtin/skills', writable: false },
    { scope: 'user', path: '/punkdom/user/skills', writable: true },
    { scope: 'workspace', path: '/books/demo/.punkdom/skills', writable: true },
  ],
  skills: [
    {
      name: 'outline',
      ...
      scope: 'workspace',
      path: '/books/demo/.punkdom/skills/outline/SKILL.md',
      editable: true,
      active: true,
      agent: 'ide',
    },
...
describe('SkillsView', () => {
  ...
  it('opens a guided create panel from the Skills page', async () => {
    ...
    expect(screen.getByText('/books/demo/.punkdom/skills/skill-name/SKILL.md')).toBeInTheDocument()
  })
  ...
  it('creates a skill with scope, description, and selected agents, then opens it', async () => {
    ...
    const createdDoc: SkillDocument = {
      ...
      scope: 'workspace',
      path: '/books/demo/.punkdom/skills/beats/SKILL.md',
      editable: true,
      active: true,
...
`,
  ...
  it('disables creation when no writable scope exists', async () => {
    ...
    vi.mocked(getSkills).mockResolvedValue({
      scopes: [{ scope: 'builtin', path: '/punkdom/builtin/skills', writable: false }],
      skills: [],
    })
    ...
    expect(screen.getByText('当前没有可写的 Skill 目录。请检查工作区或用户级 Punkdom 目录配置后再创建。')).toBeInTheDocument()
