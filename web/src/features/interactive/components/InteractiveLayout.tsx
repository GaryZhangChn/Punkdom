...
import { punkdomEase, panelPresence, subtlePresence } from '@/features/motion/motion-tokens'
...
export function InteractiveLayout({ workspace, styleSuggestions = [], loreEmpty = false, onRequestLoreInit, rightPanelVisible = true, onToggleRightPanel }: InteractiveLayoutProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[var(--punkdom-bg)] text-[var(--punkdom-text)]">
      <div data-testid="interactive-shell" className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--punkdom-bg)]">
        <div className="flex min-h-0 flex-1">
          <div className="flex min-w-0 flex-1 flex-col bg-[var(--punkdom-surface-2)]">
            <motion.div key={contentKey} variants={panelPresence} initial="initial" animate="animate" transition={{ duration: 0.2, ease: punkdomEase }} className="flex min-h-0 flex-1 flex-col">
              {submode === 'memory' ? (
                ...
                <Group id="punkdom-interactive-horizontal" defaultLayout={readStoredLayout('punkdom-interactive-horizontal')} onLayoutChanged={(layout) => storeLayout('punkdom-interactive-horizontal', layout)} orientation="horizontal" className="min-h-0 flex-1">
                  ...
                  {rightPanelVisible && (
                    <>
                      ...
                      <Panel id="snapshot" defaultSize="320px" minSize="180px" maxSize="45%" className="min-w-0">
                        <motion.div className="h-full min-h-0" variants={subtlePresence} initial="initial" animate="animate" transition={{ duration: 0.16, ease: punkdomEase }}>
                          <MemoryPanel storyId={currentStoryId} branchId={currentBranchId} snapshot={displaySnapshot} loading={snapshotPending} refreshKey={`${displaySnapshot?.current_turn?.id || ''}:${displaySnapshot?.current_turn?.memory_status || ''}:${displaySnapshot?.current_turn?.state_status || ''}`} onOpenMemoryManager={openMemoryManager} />
                        </motion.div>
                      </Panel>
                    </>
...
function MobileSnapshotDrawer({ open, title, closeLabel, onClose, children }: { open: boolean; title: string; closeLabel: string; onClose: () => void; children: ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      <button type="button" className="punkdom-mobile-drawer-backdrop absolute inset-0 bg-black/45" aria-label={closeLabel} onClick={onClose} />
      <aside role="dialog" aria-modal="true" aria-label={title} className="relative z-10 flex h-full min-h-0 w-[min(92vw,420px)] flex-col border-l border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] shadow-[var(--punkdom-shadow)]">
        <div className="punkdom-topbar flex h-11 shrink-0 items-center justify-between border-b border-[var(--punkdom-border)] px-3">
          <span className="min-w-0 truncate text-xs font-semibold text-[var(--punkdom-text)]">{title}</span>
          <button type="button" className="punkdom-icon-button flex h-8 w-8 items-center justify-center rounded-[var(--punkdom-radius)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)]" aria-label={closeLabel} onClick={onClose}>
            <X className="h-4 w-4" />
          </button>
        </div>
...
function InteractiveResizeHandle({ direction, label, prominent = false }: { direction: 'horizontal' | 'vertical'; label: string; prominent?: boolean }) {
  ...
  const className = direction === 'vertical' ? 'punkdom-resize-handle group -mx-1 flex w-3 cursor-col-resize items-center justify-center bg-transparent transition-colors' : `punkdom-resize-handle group ${prominent ? '-my-0.5 h-4' : '-my-1 h-3'} flex cursor-row-resize items-center justify-center bg-transparent transition-colors`

  return (
    <Separator aria-label={label} className={className}>
      <span className={`flex items-center justify-center rounded-full border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] text-[var(--punkdom-text-faint)] shadow-[0_4px_14px_rgba(0,0,0,0.22)] transition-colors group-hover:border-[var(--punkdom-active)] group-data-[resize-handle-active]:border-[var(--punkdom-active)] group-data-[resize-handle-active]:text-[var(--punkdom-text)] ${direction === 'vertical' ? 'h-9 w-2.5' : 'h-2.5 w-16'}`}>
