...
export function StoryStage({ workspace, styleSuggestions = [], stories = [], story, tellers = [], storyId, branchId, snapshot, snapshotLoading = false, loreEmpty = false, bookOpeningPresets = [], sceneMemoryVisible = true, onStorySelect = noop, onStoryCreate = noop, onStoryDelete = noop, onTellerChange = noop, onReplyTargetCharsChange, onRequestLoreInit, onToggleSceneMemory, onDone }: StoryStageProps) {
  ...
  const stageTextStyle = useMemo<CSSProperties>(
    () => ({
      fontSize: `var(--punkdom-reading-font-size, ${DEFAULT_READING_FONT_SIZE}px)`,
      lineHeight: stagePreferences.lineHeight,
      fontFamily: 'var(--punkdom-reading-font-family)',
    }),
    [stagePreferences.lineHeight],
  )
  const inputTextStyle = useMemo<CSSProperties>(
    () => ({
      fontSize: `min(var(--punkdom-reading-font-size, ${DEFAULT_READING_FONT_SIZE}px), 16px)`,
      lineHeight: 1.35,
      fontFamily: 'var(--punkdom-reading-font-family)',
    }),
    [],
  ...
  return (
    <main className="relative flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-[var(--punkdom-surface-2)]">
      <div data-testid="story-stage-card" className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--punkdom-surface-2)]">
        <div className="punkdom-story-stage-header punkdom-topbar flex min-h-14 flex-wrap items-center justify-between gap-3 border-b px-4 py-2">
          <div className="punkdom-story-stage-title min-w-0">
            <div className="text-[10px] font-medium leading-4 text-[var(--punkdom-text-faint)]">{t('storyStage.branchLabel', { branch: branchId || 'main' })}</div>
            <div className="truncate text-xs font-semibold leading-5 text-[var(--punkdom-text)]">{title}</div>
          </div>
          <div className="punkdom-story-stage-controls flex min-w-0 flex-wrap items-center justify-end gap-2">
            ...
            {onToggleSceneMemory && (
              <Button type="button" variant="outline" size="sm" className={`h-7 gap-1.5 border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-2 text-[11px] hover:bg-[var(--punkdom-hover)] ${sceneMemoryVisible ? 'text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)]'}`} onClick={onToggleSceneMemory} aria-label={sceneMemoryVisible ? t('storyStage.hideSceneMemory') : t('storyStage.showSceneMemory')} title={sceneMemoryVisible ? t('storyStage.hideSceneMemory') : t('storyStage.showSceneMemory')}>
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden bg-[var(--punkdom-surface-2)]">
          <section className="flex min-h-0 min-w-0 flex-1 flex-col bg-[var(--punkdom-surface-2)]">
            {snapshotLoading && messages.length === 0 && !streaming ? (
              <div className="m-5 flex min-h-0 flex-1 items-center justify-center rounded-[var(--punkdom-radius)] border border-dashed border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-6 text-center text-sm text-[var(--punkdom-text-faint)] shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                <div className="flex max-w-md flex-col items-center gap-3">
                  <RefreshCw className="h-4 w-4 animate-spin text-[var(--punkdom-text-muted)]" />
                  <div className="text-xs leading-5 text-[var(--punkdom-text-faint)]">{t('common.loading')}</div>
                </div>
              </div>
              ...
              <div className="m-5 flex min-h-0 flex-1 items-center justify-center rounded-[var(--punkdom-radius)] border border-dashed border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-6 text-center text-sm text-[var(--punkdom-text-faint)] shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                <div className="flex w-full max-w-xl flex-col items-center gap-3">
                  ...
                  <div className="space-y-1">
                    <div className="text-sm font-medium text-[var(--punkdom-text)]">{t('storyStage.opening.emptyTitle')}</div>
                    <div className="text-xs leading-5 text-[var(--punkdom-text-faint)]">{t('storyStage.opening.emptyDescription')}</div>
                  </div>
                  {loreEmpty && onRequestLoreInit ? (
                    <div className="rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 py-2 text-center">
                      <div className="text-xs font-medium text-[var(--punkdom-text)]">{t('loreInit.interactiveTitle')}</div>
                      <div className="mt-1 text-[11px] leading-5 text-[var(--punkdom-text-faint)]">{t('loreInit.interactiveDescription')}</div>
                      <button type="button" className="punkdom-nav-item mt-2 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-3 py-1.5 text-xs text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)]" onClick={onRequestLoreInit}>
                  ...
                  <div className="grid w-full gap-2 sm:grid-cols-2">
                    ...
                    <div className="flex min-w-0 gap-2">
                      <Select value={selectedBookOpeningPreset?.id || ''} onValueChange={setSelectedBookOpeningPresetId} disabled={availableBookOpeningPresets.length === 0}>
                        <SelectTrigger size="sm" className="punkdom-field min-w-0 flex-1 px-3 py-0.5 text-xs focus:ring-0" aria-label={t('storyStage.opening.bookPresetSelect')}>
                          <SelectValue placeholder={t('storyStage.opening.bookPresetSelect')} />
                        </SelectTrigger>
                        <SelectContent className="punkdom-panel border text-[var(--punkdom-text)]">
                      ...
                      <Button type="button" variant="outline" size="sm" className="shrink-0 gap-1.5 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" disabled={!storyId || streaming || !selectedBookOpeningPreset} onClick={() => startOpening('book_preset')}>
                        <BookOpen className="h-3.5 w-3.5" />
                        {t('storyStage.opening.startBookPreset')}
                      </Button>
                    </div>
                  </div>
                  {availableBookOpeningPresets.length === 0 ? <div className="text-[11px] leading-4 text-[var(--punkdom-text-faint)]">{t('storyStage.opening.bookPresetMissing')}</div> : null}
                  <div className="w-full space-y-2">
                    <Textarea autoResize className="punkdom-field min-h-20 resize-none text-xs" placeholder={t('storyStage.opening.customPlaceholder')} value={customOpeningText} onChange={(event) => setCustomOpeningText(event.target.value)} />
                    <Button type="button" variant="outline" size="sm" className="gap-1.5 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" disabled={!storyId || streaming || !customOpeningText.trim()} onClick={() => startOpening('custom')}>
      ...
      <div className="shrink-0 border-t border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] p-3">
        <div className="mx-auto max-w-5xl">
          {editingTurn && !streaming ? (
            <div className="mb-3 flex min-w-0 items-center gap-2 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 py-2 text-xs text-[var(--punkdom-text-muted)]">
              <Pencil className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
              <span className="min-w-0 flex-1 truncate">{t('storyStage.editingNotice')}</span>
              <Button type="button" variant="ghost" size="icon-xs" className="h-7 w-7 shrink-0 text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text)]" onClick={cancelEditing} aria-label={t('storyStage.cancelEdit')}>
                <X className="h-3.5 w-3.5" />
              </Button>
            </div>
          ) : null}
          {showHotChoices ? (
            <div className="mb-2 overflow-hidden rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)]">
              <div className="flex min-h-8 items-center gap-1.5 px-2 py-1 text-[11px] text-[var(--punkdom-text-muted)]">
                <button type="button" className="punkdom-nav-item flex min-w-0 flex-1 items-center gap-1.5 rounded-[var(--punkdom-radius)] px-1.5 py-1 text-left hover:bg-[var(--punkdom-hover)]" onMouseDown={(event) => event.preventDefault()} onClick={() => setHotChoicesExpanded((value) => !value)} aria-expanded={hotChoicesExpanded}>
                  <Compass className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
                  <span className="shrink-0 font-medium text-[var(--punkdom-text-muted)]">{t('storyStage.hotChoices.title')}</span>
                  <span className="min-w-0 flex-1 truncate text-[var(--punkdom-text-faint)]">
                  ...
                  {hotChoicesExpanded ? <ChevronUp className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" /> : <ChevronDown className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />}
                </button>
                {!hotChoicesLoading && (hotChoices.length === 0 || hotChoices.length < 10) ? (
                  <button type="button" className="punkdom-nav-item inline-flex h-7 shrink-0 items-center gap-1 rounded-[var(--punkdom-radius)] px-2 text-[11px] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)] disabled:opacity-50" onMouseDown={(event) => event.preventDefault()} onClick={() => requestHotChoices(hotChoices.length > 0)}>
              ...
              {hotChoicesExpanded ? (
                <div className="border-t border-[var(--punkdom-border)] px-2 py-2">
                  {hotChoicesLoading && hotChoices.length === 0 ? (
                    <div className="px-1 py-1 text-xs text-[var(--punkdom-text-faint)]">{t('storyStage.hotChoices.generatingLong')}</div>
                  ) : hotChoices.length === 0 ? (
                    <div className="px-1 py-1 text-xs text-[var(--punkdom-text-faint)]">{t('storyStage.hotChoices.emptyLong')}</div>
                  ) : (
                    <div className="flex max-h-24 flex-wrap gap-1.5 overflow-y-auto pr-1">
                      {hotChoices.map((choice, index) => (
                        <button
                          key={`${index}-${choice}`}
                          type="button"
                          className="max-w-full rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-2 py-1 text-left text-xs leading-5 text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]"
          ...
          <div className="punkdom-story-stage-composer-row flex items-center gap-3">
            <div className="relative min-w-0 flex-1">
              ...
              <Popover open={showSkillCommands && filteredSkillCommands.length > 0}>
                ...
                <PopoverContent align="start" side="top" className="punkdom-command-menu mb-2 w-[384px] overflow-hidden rounded-lg border border-[var(--punkdom-border)] p-0 text-[var(--punkdom-text)]" onOpenAutoFocus={(event) => event.preventDefault()}>
                  <Command shouldFilter={false} className="bg-transparent">
                    <div className="border-b border-[var(--punkdom-border-soft)] px-3 py-2">
                      <div className="flex items-center gap-2">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)]">
                          <CommandIcon className="h-3.5 w-3.5" />
                        </span>
                        <div className="min-w-0">
                          <div className="text-xs font-medium text-[var(--punkdom-text)]">{t('chat.commands.title')}</div>
                          <div className="text-[11px] text-[var(--punkdom-text-faint)]">{t('chat.commands.description')}</div>
                        </div>
                      </div>
                    </div>
                    <CommandList className="max-h-[312px] p-1.5">
                      <CommandEmpty className="py-5 text-center text-xs text-[var(--punkdom-text-faint)]">{t('chat.commands.empty')}</CommandEmpty>
                      <CommandGroup heading={t('chat.commands.group')} className="[&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:pb-1 [&_[cmdk-group-heading]]:pt-1 [&_[cmdk-group-heading]]:text-[11px] [&_[cmdk-group-heading]]:text-[var(--punkdom-text-faint)]">
                        {filteredSkillCommands.map((skill, index) => {
                          ...
                          return (
                            <CommandItem
                              ...
                              className={`group min-h-12 cursor-pointer rounded-md border px-2.5 py-2 text-[var(--punkdom-text-muted)] ${active ? 'border-[var(--punkdom-border)] bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'border-transparent hover:border-[var(--punkdom-border)] hover:bg-[var(--punkdom-hover)]'}`}
                            >
                              <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-[var(--punkdom-surface-2)] ${active ? 'border-[var(--punkdom-border)] text-[var(--punkdom-text)]' : 'border-[var(--punkdom-border)] text-[var(--punkdom-text-faint)]'}`}>
                                {skill.builtIn ? <Archive className="h-3.5 w-3.5" /> : <Sparkles className="h-3.5 w-3.5" />}
                              </span>
                              <span className="min-w-0 flex-1">
                                <span className="flex items-center gap-2">
                                  <span className="font-mono text-xs text-[var(--punkdom-text)]">/{skill.name}</span>
                                  <span className="truncate text-xs text-[var(--punkdom-text-muted)]">{skill.description || skill.name}</span>
                                </span>
                                <span className="mt-0.5 block text-[11px] text-[var(--punkdom-text-faint)]">{skill.hint}</span>
                              </span>
                            </CommandItem>
                          )
                        })}
              ...
              <div className="flex items-end gap-2">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      ...
                      variant="outline"
                      size="icon-sm"
                      className="h-11 w-11 shrink-0 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]"
                  ...
                  <DropdownMenuContent align="start" side="top" className="min-w-44 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text)]">
                    <DropdownMenuItem
                      disabled={!storyId || streaming}
                      onSelect={openContextAnalysis}
                      className="cursor-pointer text-xs focus:bg-[var(--punkdom-active)] focus:text-[var(--punkdom-text)]"
                    >
                ...
                <Textarea
                  ref={inputRef}
                  autoResize
                  className="punkdom-field min-h-11 flex-1 resize-none px-3 py-2 text-sm placeholder:text-[var(--punkdom-text-faint)] focus-visible:ring-1 focus-visible:ring-[var(--punkdom-border)]/35"
            ...
            {stagePreferences.hotChoicesEnabled ? (
              <Button type="button" variant="outline" className={`h-11 min-w-[4.5rem] shrink-0 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-3 text-xs text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)] ${hotChoicesExpanded ? 'text-[var(--punkdom-text)]' : ''}`} disabled={!storyId || streaming || Boolean(editingTurn)} onMouseDown={(event) => event.preventDefault()} onClick={toggleHotChoices} aria-label={hotChoicesExpanded ? t('storyStage.hotChoices.collapse') : t('storyStage.hotChoices.get')} title={hotChoicesExpanded ? t('storyStage.hotChoices.collapse') : t('storyStage.hotChoices.get')}>
                <Compass className={`h-3.5 w-3.5 ${hotChoicesLoading ? 'animate-pulse' : ''}`} />
                {t('storyStage.hotChoices.button')}
              </Button>
            ) : null}
            <Button
              className={`h-11 w-20 border border-[var(--punkdom-border)] text-[var(--punkdom-text)] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] ${streaming ? 'bg-[var(--punkdom-danger-bg)] hover:bg-[var(--punkdom-danger-bg)]' : 'bg-[var(--punkdom-active)] hover:bg-[var(--punkdom-hover)]'}`}
...
function ReplyTargetCharsControl({ story, onChange }: { story?: StorySummary; onChange?: (replyTargetChars: number) => void | Promise<void> }) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button type="button" variant="outline" size="sm" disabled={!story || !onChange} className="h-7 gap-1.5 border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-2 text-[11px] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" aria-label={t('storyStage.replyTarget.open')}>
          <Pencil className="h-3.5 w-3.5 text-[var(--punkdom-text-faint)]" />
          {t('storyStage.replyTarget.compact', { count: currentValue })}
        </Button>
      </PopoverTrigger>
      <PopoverContent align="end" sideOffset={6} className="punkdom-panel w-64 border border-[var(--punkdom-border)] p-3 text-[var(--punkdom-text)] shadow-[var(--punkdom-shadow)]">
        <div className="mb-2 text-xs font-medium">{t('storyStage.replyTarget.title')}</div>
        <Input
          className="punkdom-field text-xs"
          type="number"
          min={1}
          value={draft}
          onChange={(event) => {
        ...
        />
        {error && <div className="mt-2 text-[11px] leading-4 text-[var(--punkdom-danger)]">{error}</div>}
...
function useStagePreferences() {
  ...
  useEffect(() => {
    void load()
    window.addEventListener('punkdom:settings-updated', load)
    return () => window.removeEventListener('punkdom:settings-updated', load)
  }, [load])
