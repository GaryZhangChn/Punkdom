...
export function SettingPanel({ mode, workspace = '', tellers: externalTellers = EMPTY_TELLERS, onTellersChange, embedded = false }: SettingPanelProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('punkdom:lore-updated', onLoreUpdated)
    return () => window.removeEventListener('punkdom:lore-updated', onLoreUpdated)
  }, [])
  ...
  return (
    <section className="flex h-full min-h-0 bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text)]">
      <aside className={`punkdom-sidebar flex shrink-0 flex-col border-r ${embedded ? 'w-56' : 'w-[320px]'}`}>
        <div className="border-b border-[var(--punkdom-border)] px-3 py-3">
          <div className="flex items-center gap-2">
            <ModeIcon mode={activeMode} />
            <div className="text-sm font-semibold text-[var(--punkdom-text)]">{panelTitle(activeMode, t)}</div>
          </div>
          <div className="mt-1 text-[11px] text-[var(--punkdom-text-faint)]">{t('settingPanel.directoryHint')}</div>
        </div>
      ...
      <main className="flex min-w-0 flex-1 flex-col bg-[var(--punkdom-surface-2)]">
        <div className="punkdom-topbar flex min-h-12 shrink-0 items-center justify-between gap-3 border-b px-4">
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              {isCreatorActive ? <BookMarked className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" /> : isOpeningPresetActive ? <Sparkles className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" /> : <ModeIcon mode={activeMode} />}
              <h2 className="truncate text-sm font-semibold text-[var(--punkdom-text)]">{isLoreConfigAgentActive ? t('settingPanel.loreAgent.title') : isTellerConfigAgentActive ? t('settingPanel.tellerAgent.title') : isCreatorActive ? CREATOR_PATH : isOpeningPresetActive ? t('settingPanel.openingPreset.title') : editorTitle(activeMode, draft, tellerDraft, t)}</h2>
            </div>
            <p className="mt-0.5 truncate text-[11px] text-[var(--punkdom-text-faint)]">{isLoreConfigAgentActive ? t('settingPanel.loreAgent.subtitle') : isTellerConfigAgentActive ? t('settingPanel.tellerAgent.subtitle') : isCreatorActive ? t('settingPanel.editor.creatorSubtitle') : isOpeningPresetActive ? t('settingPanel.openingPreset.subtitle') : editorSubtitle(activeMode, draft, tellerDraft, t)}</p>
          </div>
...
function notifyLoreUpdated(itemIds: string[] = []) {
  ...
  window.dispatchEvent(new CustomEvent('punkdom:lore-updated', { detail: { item_ids: itemIds } }))
}
...
function ModeIcon({ mode }: { mode: SettingPanelMode }) {
  if (mode === 'creator') return <BookMarked className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" />
  if (mode === 'teller') return <SlidersHorizontal className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" />
  return <Database className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-muted)]" />
}
