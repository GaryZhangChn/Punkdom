...
export function BranchTimeline({
  ...
  return (
    <div className={`${workspaceMode ? 'h-full min-h-0 border-0 p-4' : `${fill ? 'h-full min-h-0' : expanded ? 'h-[min(260px,calc(100vh-96px))] min-h-[180px]' : 'h-[48px]'} border-t px-3 py-2 transition-[height] sm:px-4`} flex flex-col border-[var(--punkdom-border)] bg-[var(--punkdom-surface)]`}>
      <div className="flex items-center justify-between gap-2 text-xs text-[var(--punkdom-text-faint)]">
        {workspaceMode ? (
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <div className="flex items-center gap-1.5 rounded-[var(--punkdom-radius)] px-1.5 py-1 font-medium text-[var(--punkdom-text)]">
              <GitBranch className="h-3.5 w-3.5 text-[var(--punkdom-accent-blue)]" />
              {t('branchTimeline.title')}
            </div>
            {headerControls}
            {onBackToStory && (
              <Button variant="outline" size="xs" className="punkdom-nav-item gap-1.5 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" onClick={onBackToStory}>
          ...
          <button type="button" className="punkdom-nav-item flex items-center gap-1.5 rounded-[var(--punkdom-radius)] px-1.5 py-1 font-medium text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)]" onClick={() => setExpanded(!expanded)}>
            <GitBranch className="h-3.5 w-3.5 text-[var(--punkdom-accent-blue)]" />
            {t('branchTimeline.title')}
        ...
        <div className="flex min-w-0 flex-1 items-center justify-end gap-2 overflow-hidden">
          <span className="truncate text-[var(--punkdom-text-faint)]">{t('branchTimeline.nodeCount', { count: graphNodes.length || snapshot?.turns?.length || 0 })}</span>
          {emptyBranchCount > 0 && <Badge variant="outline" className="hidden border-[var(--punkdom-accent)]/35 bg-[var(--punkdom-accent)]/10 text-[var(--punkdom-accent)] sm:inline-flex">{t('branchTimeline.emptyBranchCount', { count: emptyBranchCount })}</Badge>}
          {selectedNode && (
            <Button variant="outline" size="xs" className="punkdom-nav-item hidden gap-1.5 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)] sm:inline-flex" onClick={openCreateDialog}>
      ...
      {expanded && (
        <div className="mt-2 flex min-h-0 flex-1 flex-col overflow-hidden rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] shadow-[var(--punkdom-shadow)]">
          <div className="punkdom-topbar flex min-h-10 shrink-0 flex-wrap items-center justify-between gap-2 border-b px-3 py-1.5 sm:px-4">
            <div className="flex min-w-0 flex-1 items-center gap-2 overflow-x-auto">
              {layout.rows.map((row) => (
                <button
                  key={row.branchId}
                  type="button"
                  className={`punkdom-nav-item flex h-7 shrink-0 items-center gap-2 rounded-[var(--punkdom-radius)] border px-2 text-xs transition ${row.branchId === currentBranchId ? 'is-active text-[var(--punkdom-text)]' : 'border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)]'}`}
                ...
                >
                  ...
                  <span className="text-[var(--punkdom-text-faint)]">{row.nodes.length}</span>
                </button>
              ))}
              {layout.rows.length === 0 && <span className="text-xs text-[var(--punkdom-text-faint)]">{t('branchTimeline.noRoutes')}</span>}
            </div>
            <div className="flex shrink-0 items-center gap-2 text-[var(--punkdom-text-faint)]">
              <span className="hidden items-center gap-1.5 text-xs sm:flex">
                <Move className="h-3.5 w-3.5" />
                {t('branchTimeline.dragHint')}
              </span>
              <Button size="xs" variant="outline" className="punkdom-nav-item gap-1.5 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]" disabled={!selectedNode} onClick={openCreateDialog}>
                <Plus className="h-3.5 w-3.5 text-[var(--punkdom-text-faint)]" />
                {t('branchTimeline.createBranch')}
              </Button>
            </div>
          </div>

          <div ref={scrollRef} className="min-h-0 flex-1 cursor-grab select-none overflow-auto overscroll-contain bg-[var(--punkdom-surface-2)] touch-none active:cursor-grabbing" data-testid="branch-graph-scroll">
            <div
            ...
            >
              ...
              {layout.positionedNodes.map(({ node, x, y, color, colorSoft }) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 overflow-hidden">
                    ...
                    <span className="mt-0.5 block truncate text-[11px] leading-4 text-[var(--punkdom-text-faint)]">{node.summary || t('branchTimeline.nodeFallback')}</span>
                  </span>
                  {node.head && <Badge variant="outline" className="h-5 max-w-12 shrink-0 border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-1.5 text-[10px] text-[var(--punkdom-text-muted)]">HEAD</Badge>}
                </button>
              ))}
              ...
              {layout.emptyBranches.map((empty) => (
                <div
                  key={empty.branch.id}
                  className="absolute z-10 flex h-[38px] cursor-grab items-center gap-2 rounded-[var(--punkdom-radius)] border border-dashed px-3 text-xs text-[var(--punkdom-text-muted)] active:cursor-grabbing"
                ...
                >
                  ...
                  <button
                    type="button"
                    data-no-drag
                    className="rounded p-1 text-[var(--punkdom-danger)] opacity-75 hover:bg-[var(--punkdom-danger-bg)] hover:opacity-100"
              ...
              {layout.rows.length === 0 && <span className="absolute left-6 top-6 text-xs text-[var(--punkdom-text-faint)]">{t('branchTimeline.noNodes')}</span>}
            </div>
          </div>
      ...
      <Dialog open={createDialogOpen} onOpenChange={handleCreateDialogOpenChange}>
        <DialogContent className="punkdom-panel border text-[var(--punkdom-text)]">
          <DialogHeader>
            <DialogTitle>{t('branchTimeline.dialogTitle')}</DialogTitle>
            <DialogDescription className="text-[var(--punkdom-text-muted)]">
              {createSourceNode ? t('branchTimeline.dialogDescription', { title: createSourceNode.title }) : ''}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <Input className="punkdom-field text-sm" value={branchTitle} onChange={(event) => setBranchTitle(event.target.value)} placeholder={t('branchTimeline.namePlaceholder')} />
            {createSourceNode?.summary && <div className="rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] p-2 text-xs leading-5 text-[var(--punkdom-text-muted)]">{createSourceNode.summary}</div>}
            {createError && <div className="rounded-[var(--punkdom-radius)] border border-[var(--punkdom-danger-border)] bg-[var(--punkdom-danger-bg)] p-2 text-xs text-[var(--punkdom-danger)]">{createError}</div>}
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => handleCreateDialogOpenChange(false)} disabled={creatingBranch}>{t('common.cancel')}</Button>
            <Button className="gap-1.5 border border-[var(--punkdom-border)] bg-[var(--punkdom-active)] text-[var(--punkdom-text)] hover:bg-[var(--punkdom-hover)]" onClick={submitCreateBranch} disabled={!createSourceNode || creatingBranch}>
