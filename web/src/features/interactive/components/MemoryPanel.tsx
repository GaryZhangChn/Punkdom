...
export function MemoryPanel({ storyId, branchId, snapshot, loading = false, refreshKey, onOpenMemoryManager }: MemoryPanelProps) {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col border-l border-[var(--punkdom-border)] bg-[var(--punkdom-surface)]">
      <div className="shrink-0 border-b border-[var(--punkdom-border)] px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div data-testid="memory-panel-icon" className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[var(--punkdom-radius)] text-[var(--punkdom-text-muted)]" aria-label={t('memoryPanel.title')} title={t('memoryPanel.title')}>
            <Brain className="h-4 w-4" />
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <div className="flex h-7 min-w-0 shrink-0 items-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-0.5" aria-label={t('memoryPanel.panelSwitch')}>
              <button
                type="button"
                onClick={() => setView('content')}
                className={`rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${view === 'content' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
              >
              ...
              <button
                ...
                className={`rounded-[6px] px-2 py-0.5 text-[11px] transition-colors ${view === 'generation' ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-faint)] hover:text-[var(--punkdom-text-muted)]'}`}
              >
        ...
        {view === 'content' ? (
          <div className="mt-3 flex items-center gap-2">
            <label className="flex min-w-0 flex-1 items-center gap-2 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2 py-1.5 text-xs text-[var(--punkdom-text-muted)]">
              <Search className="h-3.5 w-3.5 shrink-0" />
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('memoryPanel.search')} className="min-w-0 flex-1 bg-transparent text-[var(--punkdom-text)] outline-none placeholder:text-[var(--punkdom-text-faint)]" />
            </label>
            <button type="button" className="punkdom-icon-button flex h-8 w-8 items-center justify-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)]" aria-label={showArchived ? t('memoryPanel.hideArchived') : t('memoryPanel.showArchived')} onClick={() => setShowArchived((value) => !value)}>
              {showArchived ? <RotateCcw className="h-4 w-4" /> : <Archive className="h-4 w-4" />}
            </button>
            <button type="button" className="punkdom-icon-button flex h-8 w-8 items-center justify-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)] disabled:cursor-not-allowed disabled:opacity-60" aria-label={t('memoryPanel.generate')} onClick={() => void runStoryMemoryGenerate()} disabled={generating || !storyId}>
              {generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
            </button>
            <button type="button" className="punkdom-icon-button flex h-8 w-8 items-center justify-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)] disabled:cursor-not-allowed disabled:opacity-60" aria-label={t('memoryPanel.openManager')} onClick={onOpenMemoryManager} disabled={!onOpenMemoryManager}>
              <Edit3 className="h-4 w-4" />
            </button>
          </div>
        ) : (
          <div className="mt-3 flex items-center gap-2">
            <div className="flex min-w-0 flex-1 items-center gap-2 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2 py-1.5 text-xs text-[var(--punkdom-text-muted)]">
              <Brain className="h-3.5 w-3.5 shrink-0" />
              <span className="min-w-0 truncate">{t('memoryPanel.generateLog')}</span>
            </div>
            {generating ? (
              <button type="button" className="punkdom-icon-button flex h-8 w-8 items-center justify-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)]" aria-label={t('memoryPanel.abortGenerate')} onClick={abortLocalStream}>
                <X className="h-4 w-4" />
              </button>
            ) : (
              <button type="button" className="punkdom-icon-button flex h-8 w-8 items-center justify-center rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)] disabled:cursor-not-allowed disabled:opacity-60" aria-label={t('memoryPanel.generate')} onClick={() => void runStoryMemoryGenerate()} disabled={!storyId}>
                <RefreshCw className="h-4 w-4" />
              </button>
            )}
          </div>
        )}
        {error && <p className="mt-2 text-xs text-[var(--punkdom-danger)]">{error}</p>}
      </div>
      ...
      {view === 'generation' ? (
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-4 py-3">
          {generateMessages.length > 0 || generating ? (
            ...
            <div className="flex h-full min-h-[160px] items-center justify-center rounded-[var(--punkdom-radius)] border border-dashed border-[var(--punkdom-border)] px-4 text-center text-xs text-[var(--punkdom-text-muted)]">{t('memoryPanel.generateEmpty')}</div>
          )}
        </div>
      ) : (
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
          ...
          {memoryLoading ? (
            <div className="flex min-h-[160px] items-center justify-center rounded-[var(--punkdom-radius)] border border-dashed border-[var(--punkdom-border)] px-4 text-center text-xs text-[var(--punkdom-text-muted)]">{t('memoryPanel.loading')}</div>
          ) : filteredRecords.length === 0 ? (
            <div className="flex min-h-[160px] items-center justify-center rounded-[var(--punkdom-radius)] border border-dashed border-[var(--punkdom-border)] px-4 text-center text-xs text-[var(--punkdom-text-muted)]">{query.trim() ? t('memoryPanel.noMatches') : t('memoryPanel.empty')}</div>
          ) : (
                ...
                if (records.length === 0) {
                  ...
                  return (
                    <section key={structure.id} className="space-y-2">
                      ...
                      <div className="rounded-[var(--punkdom-radius)] border border-dashed border-[var(--punkdom-border)] px-3 py-6 text-center text-xs text-[var(--punkdom-text-muted)]">{t('memoryPanel.tableEmpty')}</div>
                    </section>
                  )
                }
                return (
...
function StructureTab({ active, label, count, onClick }: { active: boolean; label: string; count: number; onClick: () => void }) {
  return (
    <button
      type="button"
      className={`inline-flex h-7 max-w-[168px] shrink-0 items-center gap-1 rounded-[var(--punkdom-radius)] border px-2 text-[11px] transition-colors ${active ? 'border-[var(--punkdom-border)] bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:text-[var(--punkdom-text)]'}`}
  ...
  return (
    <div className="flex min-w-0 items-center justify-between gap-2">
      <div className="min-w-0">
        <h3 className="truncate text-xs font-semibold text-[var(--punkdom-text)]">{structure.name || structure.id}</h3>
        {structure.description && <p className="mt-0.5 line-clamp-1 break-words text-[11px] text-[var(--punkdom-text-muted)] [overflow-wrap:anywhere]">{structure.description}</p>}
      </div>
      <span className="shrink-0 rounded-full border border-[var(--punkdom-border)] px-2 py-0.5 text-[10px] text-[var(--punkdom-text-muted)]">{t('memoryPanel.recordCount', { count })}</span>
    </div>
  )
}
...
function MemoryRecordCard({ record, structure }: { record: StoryMemoryRecord; structure: StoryMemoryStructure }) {
  ...
  return (
    <article className={`rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] p-3 ${record.archived ? 'opacity-55' : ''}`}>
      <div className="min-w-0">
        <h4 className="break-words text-sm font-medium text-[var(--punkdom-text)] [overflow-wrap:anywhere]">{storyMemoryRecordTitle(record, structure, t('storyMemory.untitled'))}</h4>
      ...
      <div className="mt-2 space-y-2">
        {visibleFields.map((field) => (
          <section key={field.id} className="min-w-0">
            <div className="mb-0.5 truncate text-[11px] font-medium text-[var(--punkdom-text-muted)]">{field.name || field.id}</div>
            <p className="line-clamp-4 whitespace-pre-wrap break-words text-xs leading-5 text-[var(--punkdom-text)] [overflow-wrap:anywhere]">{recordFieldValue(record, field.id) || t('storyMemory.noValue')}</p>
          </section>
        ))}
      </div>
    </article>
...
function SyncBadge({ status, error, loading }: { status?: string; error?: string; loading?: boolean }) {
  ...
  if (loading || status === 'pending') {
    return (
      <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-[var(--punkdom-border)] px-2 py-1 text-[11px] text-[var(--punkdom-text-muted)]">
  ...
  if (status === 'failed') {
    return <span className="inline-flex max-w-[120px] shrink-0 truncate rounded-full border border-[var(--punkdom-danger)] px-2 py-1 text-[11px] text-[var(--punkdom-danger)]" title={error}>{t('memoryPanel.failed')}</span>
  }
  return <span className="inline-flex shrink-0 rounded-full border border-[var(--punkdom-border)] px-2 py-1 text-[11px] text-[var(--punkdom-text-muted)]">{t('memoryPanel.ready')}</span>
}

function MemoryChip({ children }: { children: string }) {
  return <span className="max-w-full truncate rounded-full border border-[var(--punkdom-border)] px-2 py-0.5 text-[11px] text-[var(--punkdom-text-muted)]">{children}</span>
}
