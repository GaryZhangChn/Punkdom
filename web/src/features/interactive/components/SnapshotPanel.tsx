...
export function SnapshotPanel({ snapshot, loading = false }: { snapshot: Snapshot | null; loading?: boolean }) {
  ...
  return (
    <aside className="punkdom-sidebar flex h-full min-w-0 flex-col border-l p-4">
      <div className="mb-3 flex h-8 items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold text-[var(--punkdom-text)]">{t('snapshot.title')}</h2>
          <div className="text-[11px] text-[var(--punkdom-text-faint)]">{t('snapshot.subtitle')}</div>
        </div>
        <div className="flex items-center gap-1.5">
          {loading ? <Badge variant="outline" className="gap-1 border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)]"><RefreshCw className="h-3 w-3 animate-spin" />{t('common.loading')}</Badge> : null}
          {stateStatus === 'pending' ? <Badge variant="outline" className="border-[var(--punkdom-accent)]/40 bg-[var(--punkdom-accent)]/10 text-[var(--punkdom-accent)]">{t('snapshot.syncing')}</Badge> : null}
          {stateStatus === 'failed' ? <Badge variant="outline" className="border-[var(--punkdom-danger-border)] bg-[var(--punkdom-danger-bg)] text-[var(--punkdom-danger)]">{t('snapshot.syncFailed')}</Badge> : null}
          <Badge variant="outline" className="border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)]">{formatBranchName(snapshot?.branch_id, t)}</Badge>
        </div>
      </div>
      {stateStatus === 'failed' && snapshot?.current_turn?.state_error ? (
        <div className="mb-3 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-danger-border)] bg-[var(--punkdom-danger-bg)] px-3 py-2 text-xs text-[var(--punkdom-danger)]">
      ...
      <ScrollArea className="min-h-0 flex-1 pr-1">
        <section className={panelSectionClass}>
          ...
          {sceneEntries.length ? (
            <div className="mt-3 border-t border-[var(--punkdom-border)] pt-3">
        ...
        <section className={panelSectionClass}>
          <div className="mb-2 text-xs font-semibold text-[var(--punkdom-text-muted)]">{t('snapshot.sceneNotes')}</div>
          <div className={`${panelCardClass} px-3 py-2 text-xs leading-5 text-[var(--punkdom-text-faint)]`}>
        ...
        <section className={panelSectionClass}>
          ...
          <div className="flex flex-wrap gap-1.5 text-sm text-[var(--punkdom-text-muted)]">
            {onStage.length ? onStage.map((name) => <Badge key={String(name)} className="border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text)]" variant="secondary">{String(name)}</Badge>) : t('snapshot.noOnStage')}
          </div>
        </section>
        ...
        <section className={panelSectionClass}>
          ...
          <div className="space-y-2 text-xs text-[var(--punkdom-text-muted)]">
            {characters.length ? characters.map(([name, state]) => (
              <div key={name} className={`${panelCardClass} p-2`}>
                <div className="mb-1 font-medium text-[var(--punkdom-text)]">{name}</div>
          ...
          <div className="space-y-2 text-xs text-[var(--punkdom-text-muted)]">
            {inventory ? (
              <div className={`${panelCardClass} p-2`}>
                <div className="mb-1 font-medium text-[var(--punkdom-text)]">{t('snapshot.field.inventory')}</div>
                <StateValue value={inventory} />
              </div>
            ) : null}
            {resources ? (
              <div className={`${panelCardClass} p-2`}>
                <div className="mb-1 font-medium text-[var(--punkdom-text)]">{t('snapshot.field.resources')}</div>
        ...
        <section className={panelSectionClass}>
          ...
          <div className="space-y-3 text-xs text-[var(--punkdom-text-muted)]">
        ...
        <section className={`${panelSectionClass} mb-0`}>
          <div className="mb-3 text-xs font-semibold text-[var(--punkdom-text-muted)]">{t('snapshot.quickActions')}</div>
...
function InspectorAction({ icon: Icon, label }: { icon: LucideIcon; label: string }) {
  return (
    <button type="button" className="punkdom-nav-item flex h-9 items-center justify-center gap-1.5 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-xs text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]">
...
function SnapshotMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] px-2 py-2">
      <div className="text-[10px] text-[var(--punkdom-text-faint)]">{label}</div>
      <div className="truncate text-xs font-medium text-[var(--punkdom-text)]" title={value}>{value}</div>
    </div>
  )
}
...
function StateValue({ value }: { value: unknown }) {
  ...
  if (isPlainObject(value)) {
    ...
    if (!entries.length) return <div className="text-[var(--punkdom-text-faint)]">{t('snapshot.noRecord')}</div>
    return (
      <dl className="grid gap-1.5">
        {entries.map(([key, item]) => (
          <div key={key} className="grid grid-cols-[64px_minmax(0,1fr)] gap-2">
            <dt className="truncate text-[var(--punkdom-text-faint)]" title={formatLabel(key, t)}>{formatLabel(key, t)}</dt>
            <dd className="min-w-0 text-[var(--punkdom-text-muted)]">{renderReadableValue(item, t)}</dd>
          </div>
        ))}
      </dl>
    )
  }
  return <div className="whitespace-pre-wrap text-[var(--punkdom-text-muted)]">{formatScalar(value, t)}</div>
}
...
function EventItem({ event, index }: { event: unknown; index: number }) {
  ...
  if (!isPlainObject(event)) {
    return (
      <div className={`${panelCardClass} p-2 text-[var(--punkdom-text-muted)]`}>
  ...
  return (
    <article className={`${panelCardClass} p-2`}>
      <div className="mb-1 flex items-start justify-between gap-2">
        <div className="min-w-0 font-medium text-[var(--punkdom-text)]">{title}</div>
        {typeof event.type === 'string' && event.type.trim() ? (
          <Badge variant="outline" className="h-5 shrink-0 border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-1.5 text-[10px] text-[var(--punkdom-text-muted)]">
            {event.type.trim()}
          </Badge>
        ) : null}
      </div>
      {description ? <div className="mb-2 whitespace-pre-wrap text-[var(--punkdom-text-muted)]">{description}</div> : null}
      {detailEntries.length ? (
        <dl className="grid gap-1.5">
          {detailEntries.map(([key, value]) => (
            <div key={key} className="grid grid-cols-[64px_minmax(0,1fr)] gap-2">
              <dt className="truncate text-[var(--punkdom-text-faint)]" title={formatLabel(key, t)}>{formatLabel(key, t)}</dt>
              <dd className="min-w-0 text-[var(--punkdom-text-muted)]">{renderReadableValue(value, t)}</dd>
            </div>
          ))}
        </dl>
      ) : null}
...
function CompactList({ items, empty }: { items: unknown[]; empty: string }) {
  const { t } = useTranslation()
  if (!items.length) return <div className="text-xs text-[var(--punkdom-text-muted)]">{empty}</div>
  return (
    <div className="space-y-1.5 text-xs text-[var(--punkdom-text-muted)]">
...
function LabeledList({ label, items, empty }: { label: string; items: unknown[]; empty: string }) {
  return (
    <div>
      <div className="mb-1 text-[10px] font-medium text-[var(--punkdom-text-faint)]">{label}</div>
...
function renderReadableValue(value: unknown, t: (key: string, options?: Record<string, unknown>) => string): ReactNode {
  if (Array.isArray(value)) {
    if (!value.length) return <span className="text-[var(--punkdom-text-faint)]">{t('snapshot.empty')}</span>
    if (value.every((item) => !isPlainObject(item) && !Array.isArray(item))) {
      return (
        <div className="flex flex-wrap gap-1">
          {value.map((item, index) => (
            <Badge key={index} variant="secondary" className="border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] text-[var(--punkdom-text)]">
              {formatScalar(item, t)}
            </Badge>
          ))}
        </div>
    ...
    return (
      <div className="space-y-1">
        {value.map((item, index) => (
          <div key={index} className="rounded border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-2 py-1">
            {renderReadableValue(item, t)}
          </div>
        ))}
      </div>
  ...
  if (isPlainObject(value)) {
    const entries = Object.entries(value)
    if (!entries.length) return <span className="text-[var(--punkdom-text-faint)]">{t('snapshot.empty')}</span>
    return (
      <div className="space-y-1">
        {entries.map(([key, item]) => (
          <div key={key} className="grid grid-cols-[64px_minmax(0,1fr)] gap-1">
            <span className="text-[var(--punkdom-text-faint)]">{formatLabel(key, t)}</span>
...
const panelSectionClass = 'mb-3 rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] p-3'
const panelCardClass = 'rounded-[var(--punkdom-radius)] border border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)]'
const sectionTitleClass = 'mb-3 flex items-center gap-2 text-xs font-semibold text-[var(--punkdom-text-muted)]'
