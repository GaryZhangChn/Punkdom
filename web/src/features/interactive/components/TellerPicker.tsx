...
export function TellerPicker({ story, tellers, onChange, layout = 'inline' }: TellerPickerProps) {
  ...
  const selector = (
    <Select value={story?.story_teller_id || '__none'} disabled={!story} onValueChange={(value) => { if (value !== '__none') onChange(value) }}>
      <SelectTrigger
        size="sm"
        className={`punkdom-field ${sidebar ? 'w-full' : 'w-[170px]'} px-3 py-0.5 text-xs focus:ring-0`}
  ...
  if (sidebar) {
    return (
      <div className="flex min-w-0 flex-col gap-1.5">
        <span className="shrink-0 text-[11px] font-medium text-[var(--punkdom-text-faint)]">{t('tellerPicker.label')}</span>
        {selector}
      </div>
    )
  }
