...
const iconActionClassName = 'punkdom-nav-item border-[var(--punkdom-border)] bg-[var(--punkdom-surface-2)] text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
const inputClassName = 'punkdom-field h-8 text-xs focus-visible:ring-0'
const selectClassName = 'punkdom-field h-8 text-xs focus:ring-0'
...
export function LoreDirectory({
  ...
  return (
    <>
      <div className="border-b border-[var(--punkdom-border)] p-2">
        <div className="punkdom-field flex h-8 items-center gap-2 rounded-[var(--punkdom-radius)] px-2 text-xs text-[var(--punkdom-text-faint)]">
          <Search className="h-3.5 w-3.5" />
          <input
            className="min-w-0 flex-1 bg-transparent text-[var(--punkdom-text-muted)] outline-none placeholder:text-[var(--punkdom-text-faint)]"
            value={query}
        ...
        <button
          ...
          className={`mt-2 flex h-9 w-full items-center gap-2 rounded-md px-2 text-left text-xs transition ${
            activeId === CREATOR_ENTRY_ID ? 'is-active bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
          }`}
        >
          <BookMarked className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
        ...
        <button
          ...
          className={`mt-2 flex h-9 w-full items-center gap-2 rounded-md px-2 text-left text-xs transition ${
            activeId === INTERACTIVE_OPENING_PRESET_ENTRY_ID ? 'is-active bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
          }`}
        >
          <Sparkles className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
        ...
        <button
          ...
          className={`mt-2 flex h-9 w-full items-center gap-2 rounded-md px-2 text-left text-xs transition ${
            activeId === LORE_CONFIG_AGENT_ENTRY_ID ? 'is-active bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
          }`}
        >
          <Bot className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
          ...
          {sections.map(({ section, entries }) => {
            ...
            return (
              <section key={section.id} className={entries.length ? 'mb-2' : 'mb-1'}>
                <div className={`flex h-8 items-center gap-2 rounded px-2 text-xs ${entries.length ? 'text-[var(--punkdom-text-muted)]' : 'text-[var(--punkdom-text-faint)]'}`}>
                  <button
                    type="button"
                    className="punkdom-nav-item rounded p-0.5 text-[var(--punkdom-text-faint)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]"
                  ...
                  <Icon className="h-3.5 w-3.5 text-[var(--punkdom-text-faint)]" />
                  <span className="min-w-0 flex-1 truncate font-medium">{label}</span>
                  <span className="text-[11px] text-[var(--punkdom-text-faint)]">{entries.length}</span>
                  <button
                    type="button"
                    className="punkdom-nav-item rounded p-1 text-[var(--punkdom-text-faint)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]"
                    disabled={saving}
                ...
                {!collapsed && entries.length > 0 && (
                  <div className="ml-5 space-y-0.5 border-l border-[var(--punkdom-border)] pl-2">
                    {entries.map((item) => (
                      <button
                        key={item.id}
                        type="button"
                        ...
                        className={`flex h-8 w-full items-center gap-2 rounded-md px-2 text-left text-xs transition ${
                          activeId === item.id ? 'is-active bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
                        } ${item.enabled === false ? 'opacity-50' : ''}`}
                      >
                        <FileText className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
                        <span className="min-w-0 flex-1 truncate">{item.name}</span>
                        {item.enabled === false ? <span className="shrink-0 text-[10px] text-[var(--punkdom-text-faint)]">{t('settingPanel.disabled')}</span> : null}
                      </button>
                    ))}
                  </div>
                )}
...
export function CreatorDirectory() {
  ...
  return (
    <div className="p-2">
      <div className="flex h-8 items-center gap-2 rounded px-2 text-xs text-[var(--punkdom-text-muted)]">
        <ChevronDown className="h-3.5 w-3.5 text-[var(--punkdom-text-faint)]" />
        <Folder className="h-3.5 w-3.5 text-[var(--punkdom-text-faint)]" />
        <span className="font-medium">{t('settingPanel.rootDirectory')}</span>
      </div>
      <div className="ml-5 border-l border-[var(--punkdom-border)] pl-2">
        <div className="flex h-8 items-center gap-2 rounded-[var(--punkdom-radius)] bg-[var(--punkdom-active)] px-2 text-xs text-[var(--punkdom-text)]">
          <BookMarked className="h-3.5 w-3.5 text-[var(--punkdom-text-muted)]" />
...
export function TellerDirectory({
  ...
  return (
    <>
      <div className="flex h-10 items-center justify-between border-b border-[var(--punkdom-border)] px-3">
        <div className="text-xs font-medium text-[var(--punkdom-text-muted)]">{t('settingPanel.tellerDirectory')}</div>
      ...
      <div className="border-b border-[var(--punkdom-border)] p-2">
        <button
          ...
          className={`flex h-9 w-full items-center gap-2 rounded-md px-2 text-left text-xs transition ${
            activeTellerId === TELLER_CONFIG_AGENT_ENTRY_ID ? 'is-active bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
          }`}
        >
          <Bot className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
            ...
            {tellers.map((teller) => (
              <button
                ...
                className={`flex min-h-9 w-full items-center gap-2 rounded-md px-2 py-1 text-left text-xs transition ${
                  activeTellerId === teller.id ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
                }`}
              >
                <SlidersHorizontal className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
                <span className="min-w-0 flex-1">
                  <span className="block truncate">{teller.name}</span>
                  <span className="block truncate text-[11px] text-[var(--punkdom-text-faint)]">{teller.custom ? t('settingPanel.custom') : t('settingPanel.builtIn')} · {t('settingPanel.enabledRules', { count: (teller.slots || []).filter((slot) => slot.enabled).length })}</span>
                </span>
              </button>
            ))}
          </div>
...
export function LoreEditor({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="grid shrink-0 gap-3 border-b border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] p-4 lg:grid-cols-[minmax(220px,1fr)_120px_150px_150px_170px]">
        ...
        <Field className="lg:col-span-5" label={t('settingPanel.field.brief')}>
          <Textarea
            autoResize
            className="punkdom-field min-h-[96px] resize-y text-xs leading-5 shadow-none focus-visible:ring-0"
        ...
        <div className="lg:col-span-5 text-[11px] leading-5 text-[var(--punkdom-text-faint)]">
          {draft.load_mode === 'resident' ? t('settingPanel.lore.residentDesc') : loadModeDescription(draft.load_mode, t)}
          {residentWarning ? <span className="ml-2 text-[var(--punkdom-danger)]">{t('settingPanel.lore.residentWarning')}</span> : null}
        </div>
      </div>
      <div className="min-h-0 flex-1 p-4">
        <Textarea
          className="punkdom-field h-full min-h-[360px] resize-none font-mono text-sm leading-7 shadow-none focus-visible:ring-0"
          value={draft.content || ''}
...
export function CreatorEditor({
  ...
  return (
    <div className="min-h-0 flex-1 p-4">
      <Textarea
        className="punkdom-field h-full min-h-[520px] resize-none font-mono text-sm leading-7 shadow-none focus-visible:ring-0"
        value={content}
...
export function OpeningPresetEditor({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="shrink-0 border-b border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-xs font-medium text-[var(--punkdom-text)]">{t('settingPanel.openingPreset.title')}</div>
            <div className="mt-1 text-[11px] leading-5 text-[var(--punkdom-text-faint)]">{t('settingPanel.openingPreset.description')}</div>
          </div>
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="w-56 shrink-0 border-r border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] p-2">
          {presets.length === 0 ? (
            <div className="px-2 py-3 text-xs leading-5 text-[var(--punkdom-text-faint)]">{t('settingPanel.openingPreset.empty')}</div>
          ) : (
              ...
              {presets.map((preset) => (
                <button
                  ...
                  className={`flex min-h-8 w-full items-center gap-2 rounded-md px-2 py-1 text-left text-xs transition ${
                    activePreset?.id === preset.id ? 'bg-[var(--punkdom-active)] text-[var(--punkdom-text)]' : 'text-[var(--punkdom-text-muted)] hover:bg-[var(--punkdom-hover)] hover:text-[var(--punkdom-text)]'
                  }`}
                >
                  <Sparkles className="h-3.5 w-3.5 shrink-0 text-[var(--punkdom-text-faint)]" />
        ...
        <div className="min-h-0 flex-1 p-4">
          {activePreset ? (
            <div className="flex h-full min-h-0 flex-col gap-3">
              ...
              <Textarea
                className="punkdom-field min-h-0 flex-1 resize-none text-sm leading-7 shadow-none focus-visible:ring-0"
...
function Field({ label, children, className = '' }: { label: string; children: ReactNode; className?: string }) {
  return (
    <label className={`grid gap-1.5 ${className}`}>
      <span className="text-[11px] text-[var(--punkdom-text-faint)]">{label}</span>
      {children}
    </label>
  )
}
...
function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center p-6">
      <div className="rounded-[var(--punkdom-radius)] border border-dashed border-[var(--punkdom-border)] bg-[var(--punkdom-surface)] px-6 py-5 text-center">
        <div className="text-sm font-medium text-[var(--punkdom-text)]">{title}</div>
        <div className="mt-1 text-xs text-[var(--punkdom-text-faint)]">{description}</div>
      </div>
    </div>
  )
}
