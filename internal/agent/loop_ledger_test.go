...
func TestRunTraceReaderSummarizesLedger(t *testing.T) {
	workspace := t.TempDir()
	ledger, err := newRunLedgerWithOptions(workspace, RunLedgerPolicy{Enabled: true, Directory: ".punkdom/runs", PreviewChars: 8}, RunOptions{
...
func TestRunLedgerWritesBoundedJSONLTrace(t *testing.T) {
	...
	if !strings.HasPrefix(filepath.ToSlash(ledger.Path()), filepath.ToSlash(filepath.Join(workspace, ".punkdom/runs"))) {
		t.Fatalf("ledger path should be under workspace .punkdom/runs: %s", ledger.Path())
	}
