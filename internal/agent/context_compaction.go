...
const (
	...
	contextCompactionSummaryPrefix = "[Punkdom Context Compaction]"
...
你是 Punkdom 的独立“互动小说上下文压缩器”，用于类似酒馆/SillyTavern 的高轮次互动小说和长对话创作场景。
...
func buildContextCompactionTranscript(messages []*schema.Message, referenceContext string, sourceTokens int, retryInstruction string, policy contextCompactionPolicy) string {
	...
	sb.WriteString("请按系统要求压缩以下 Punkdom 上下文。保留所有会影响后续剧情、任务、关系、世界状态或用户偏好的信息。\n")
