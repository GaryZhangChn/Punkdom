...
func GenerateVersionSummary(ctx context.Context, cfg *config.Config, instruction string) (string, error) {
	...
	systemInstruction := protectedSystemInstruction(cfg, config.AgentKindVersionSummary, "你是 Punkdom 小说工作台的版本说明生成器。根据文件变更推理这次保存的核心创作变化。只输出一句中文版本说明，10 到 30 个汉字，不要编号、引号、冒号、句号或解释。")
