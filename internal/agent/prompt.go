...
func BuiltinAgentPrompts(cfg *config.Config, state *book.State, ideTeller IDEStoryTeller) config.AgentPromptSettings {
	...
	return config.AgentPromptSettings{
		...
		VersionSummary:        config.AgentPromptOverride{SystemPrompt: protectedSystemInstruction(promptCfg, config.AgentKindVersionSummary, "你是 Punkdom 小说工作台的版本说明生成器。根据文件变更推理这次保存的核心创作变化。只输出一句中文版本说明，10 到 30 个汉字，不要编号、引号、冒号、句号或解释。")},
...
func BuiltinAgentPromptBlocks(cfg *config.Config, state *book.State, ideTeller IDEStoryTeller) config.AgentPromptBlockSettings {
	...
	return config.AgentPromptBlockSettings{
		...
		VersionSummary:        builtinPromptBlocks(config.AgentKindVersionSummary, "你是 Punkdom 小说工作台的版本说明生成器。根据文件变更推理这次保存的核心创作变化。只输出一句中文版本说明，10 到 30 个汉字，不要编号、引号、冒号、句号或解释。"),
...
func BuiltinAgentPromptSources(cfg *config.Config, state *book.State, ideTeller IDEStoryTeller) config.AgentPromptSourceSettings {
	...
	return config.AgentPromptSourceSettings{
		...
		VersionSummary:        builtinPromptSourceList(config.AgentKindVersionSummary, "你是 Punkdom 小说工作台的版本说明生成器。根据文件变更推理这次保存的核心创作变化。只输出一句中文版本说明，10 到 30 个汉字，不要编号、引号、冒号、句号或解释。"),
...
func configManagerFlowInstructionFor(workspace, creator string) string {
	...
	sb.WriteString("你是 Punkdom 的统一配置管理 Agent，负责在模块内嵌入口中帮助用户管理资料库、叙事编排、自动化任务、Skills、故事记忆结构和故事记忆记录。\n\n")
	...
	sb.WriteString(strings.Join([]string{
		...
		"- 不要修改 Punkdom 设置、模型、端口、主题、Agent prompt 或工具权限。",
...
func systemPromptSourceSummary(mode, creator, stateContext string, extraSources ...promptSource) string {
	...
	contextLog.add("系统提示", "Punkdom "+mode+" 内置规则", "基础规则、工具边界、工作流约束", "")
...
func promptStateSectionSource(title string) string {
	...
	case "章节目录概览":
		return "chapters/"
	case "资料库":
		return ".punkdom/lore/items.json"
	default:
		return "作品状态注入"
	}
}
