...
func newConfigManagerTools(cfg *config.Config) ([]tool.BaseTool, error) {
	...
	punkdomDir := strings.TrimSpace(cfg.PunkdomDir)
	workspace := strings.TrimSpace(cfg.Workspace)
	builders := []func() (tool.BaseTool, error){
		func() (tool.BaseTool, error) { return newListTellersTool(punkdomDir) },
		func() (tool.BaseTool, error) { return newReadTellersTool(punkdomDir) },
		func() (tool.BaseTool, error) { return newWriteTellersTool(punkdomDir) },
		func() (tool.BaseTool, error) { return newListAutomationsTool(punkdomDir, workspace) },
		func() (tool.BaseTool, error) { return newReadAutomationsTool(punkdomDir, workspace) },
		func() (tool.BaseTool, error) { return newWriteAutomationsTool(punkdomDir, workspace) },
...
func newListTellersTool(punkdomDir string) (tool.BaseTool, error) {
	return utils.InferTool("list_tellers", "列出叙事编排/导演索引，返回 ID、名称、简介、标签和槽位概览；需要完整配置时再调用 read_tellers。", func(ctx context.Context, input struct{}) (string, error) {
		...
		_ = input
		if punkdomDir == "" {
			return "", fmt.Errorf("punkdom_dir 不可用，无法读取叙事编排")
		}
		tellers, err := interactive.NewTellerLibrary(punkdomDir).List()
		if err != nil {
			return "", err
		}
		if len(tellers) == 0 {
...
func newWriteTellersTool(punkdomDir string) (tool.BaseTool, error) {
	return utils.InferTool("write_tellers", "批量创建、更新或删除叙事编排/导演配置。删除内置导演会被后端拒绝；删除必须来自用户明确指令。", func(ctx context.Context, input tellerWriteInput) (string, error) {
		_ = ctx
		if punkdomDir == "" {
			return "", fmt.Errorf("punkdom_dir 不可用，无法写入叙事编排")
		}
		lib := interactive.NewTellerLibrary(punkdomDir)
...
func newListAutomationsTool(punkdomDir, workspace string) (tool.BaseTool, error) {
	return utils.InferTool("list_automations", "列出自动化任务索引，返回 ID、名称、启用状态、模板、触发器和写入策略；需要完整配置时再调用 read_automations。", func(ctx context.Context, input struct{}) (string, error) {
		_ = ctx
		_ = input
		tasks, err := automation.NewStore(punkdomDir, workspace).List()
		if err != nil {
			return "", err
		}
		var sb strings.Builder
...
func newReadAutomationsTool(punkdomDir, workspace string) (tool.BaseTool, error) {
	return utils.InferTool("read_automations", "按自动化任务 ID 批量读取完整任务配置。", func(ctx context.Context, input idListInput) (string, error) {
		_ = ctx
		store := automation.NewStore(punkdomDir, workspace)
...
func newWriteAutomationsTool(punkdomDir, workspace string) (tool.BaseTool, error) {
...
func newListSkillsTool(cfg *config.Config) (tool.BaseTool, error) {
	return utils.InferTool("list_skills", "列出 Skills 索引，返回名称、scope、agent、描述、是否可编辑和是否生效；需要完整 SKILL.md 时再调用 read_skills。", func(ctx context.Context, input struct{}) (string, error) {
		_ = input
		snapshot, err := punkdomskills.SnapshotFor(ctx, skillDirs(cfg))
		if err != nil {
			return "", err
		}
		var sb strings.Builder
...
func newReadSkillsTool(cfg *config.Config) (tool.BaseTool, error) {
	return utils.InferTool("read_skills", "按 scope/name 批量读取完整 SKILL.md。", func(ctx context.Context, input readSkillsInput) (string, error) {
		docs := []punkdomskills.Document{}
		for _, item := range input.Items {
			doc, err := punkdomskills.ReadDocument(ctx, skillDirs(cfg), punkdomskills.Scope(strings.TrimSpace(item.Scope)), strings.TrimSpace(item.Name))
			if err != nil {
				return "", err
			}
...
func newWriteSkillsTool(cfg *config.Config) (tool.BaseTool, error) {
	return utils.InferTool("write_skills", "批量创建、更新或删除 Skills。scope 必须是 user 或 workspace；删除必须来自用户明确指令。", func(ctx context.Context, input skillsWriteInput) (string, error) {
		result := map[string][]string{"created": []string{}, "updated": []string{}, "deleted": []string{}}
		for _, op := range input.Operations {
			scope := punkdomskills.Scope(strings.TrimSpace(op.Scope))
			name := strings.TrimSpace(op.Name)
			switch strings.TrimSpace(op.Op) {
			case "create":
				var doc punkdomskills.Document
				var err error
				if strings.TrimSpace(op.Content) == "" {
					doc, err = punkdomskills.CreateDocument(ctx, skillDirs(cfg), scope, name, op.Description, op.Agents...)
				} else {
					doc, err = punkdomskills.SaveDocument(ctx, skillDirs(cfg), scope, name, op.Content)
				}
				if err != nil {
					return "", err
				}
			...
			case "delete":
				if err := punkdomskills.DeleteDocument(ctx, skillDirs(cfg), scope, name); err != nil {
					return "", err
				}
...
func skillDirs(cfg *config.Config) []punkdomskills.Directory {
	if cfg == nil {
		return nil
	}
	return punkdomskills.NewDirectories(cfg.SkillsDir, cfg.PunkdomDir, cfg.Workspace)
}
