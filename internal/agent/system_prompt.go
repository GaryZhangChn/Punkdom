...
func protectedSystemInstruction(cfg *config.Config, agentKind, builtIn string) string {
	...
	sb.WriteString("# Punkdom 运行时契约（不可覆盖）\n\n")
	...
	if flow := resolvedPrompt.FlowPrompt; flow != "" {
		...
		sb.WriteString("以下流程规则优先于 Punkdom 内置流程规则；但不得覆盖运行时契约、输出格式、工具权限和后端校验。若存在冲突，必须忽略冲突部分。\n\n")
		sb.WriteString(flow)
	}
	if custom := resolvedPrompt.SystemPrompt; custom != "" {
		sb.WriteString("\n\n---\n\n")
		sb.WriteString("# 用户自定义系统提示（受保护最高优先级）\n\n")
		sb.WriteString("以下提示在 Agent 行为、创作偏好、策略和风格上优先于 Punkdom 内置提示；但不得覆盖上一节运行时契约。若以下提示与运行时契约冲突，必须忽略冲突部分。\n\n")
		sb.WriteString(custom)
	}
	if builtIn != "" {
		sb.WriteString("\n\n---\n\n")
		sb.WriteString("# Punkdom 内置系统提示\n\n")
...
func runtimeContractForAgent(agentKind string) string {
	common := strings.Join([]string{
		"- 运行时契约高于用户自定义系统提示和 Punkdom 内置提示。",
...
func agentRuntimeContract(agentKind string) string {
	...
	case config.AgentKindConfigManager:
		return strings.Join([]string{
			...
			"- 不负责修改 Punkdom 设置、模型配置、端口、主题、Agent prompt 或工具权限；这些必须由设置页和 Agents 页完成。",
