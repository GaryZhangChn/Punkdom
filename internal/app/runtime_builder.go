...
func buildRuntime(ctx context.Context, cfg *config.Config, workspace string) (*runtimeState, error) {
	...
	if err := os.MkdirAll(book.UserStyleDir(cfg.PunkdomDir), 0o755); err != nil {
	...
	return &runtimeState{
		...
		bookState:              state,
		bookService:            book.NewServiceWithStyleRoot(absWorkspace, book.UserStyleDir(cfg.PunkdomDir)),
...
func ideStoryTellerForConfig(cfg *config.Config) agent.IDEStoryTeller {
	...
	teller := loadInteractiveTeller(cfg.PunkdomDir, tellerID)
	if teller.ID == "" {
		return agent.IDEStoryTeller{}
	}
	return agent.IDEStoryTeller{
