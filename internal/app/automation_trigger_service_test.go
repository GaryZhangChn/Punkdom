...
func TestAutomationCheckCreatesRetryableInboxWhenAutoRunCannotStart(t *testing.T) {
	...
	app := &App{cfg: &config.Config{PunkdomDir: filepath.Join(root, "punkdom"), Workspace: workspace}, workspace: workspace}
	app.ensureServices()
...
func TestAutomationChapterBatchTriggerCreatesInboxAtBatchBoundaries(t *testing.T) {
	...
	app := &App{cfg: &config.Config{PunkdomDir: filepath.Join(root, "punkdom"), Workspace: workspace}, workspace: workspace}
	app.ensureServices()
	app.bookService = book.NewServiceWithStyleRoot(workspace, book.UserStyleDir(app.cfg.PunkdomDir))
