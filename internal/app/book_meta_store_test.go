...
func TestBookMetaStoreWriteAndReadFromPunkdomDir(t *testing.T) {
	root := t.TempDir()
	...
	store := NewBookMetaStore(filepath.Join(root, "punkdom"))
...
func TestBookMetaStorePrefersPunkdomDirOverLegacyBookJSON(t *testing.T) {
	root := t.TempDir()
	bookDir := filepath.Join(root, "book")
	if err := os.MkdirAll(bookDir, 0o755); err != nil {
		t.Fatal(err)
