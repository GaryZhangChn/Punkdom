...
func (s *InteractiveAppService) runStoryMemoryGenerate(ctx context.Context, storyID, branchID, source string, emit func(agent.Event)) (interactive.StoryMemoryState, int, error) {
	...
	conversation := newInteractiveConversation(store, runtimeCfg.PunkdomDir, workspace, storyID, snapshot.BranchID, snapshot.CurrentTurn.User, runtimeCfg.InteractiveReplyTargetChars, &runtimeCfg)
...
func (s *InteractiveAppService) AnalyzeInteractiveContext(storyID, branchID, message string, styleReferences []string) (agent.ContextAnalysis, error) {
	...
	workspace := a.workspace
	runtimeCfg.Workspace = workspace
	punkdomDir := runtimeCfg.PunkdomDir
	a.mu.RUnlock()

	if layered, err := config.LoadLayered(punkdomDir, workspace); err == nil {
	...
	teller := loadInteractiveTeller(punkdomDir, storyCtx.Meta.StoryTellerID)
	...
	if len(styleReferences) == 0 {
		styleRules = convertTellerStyleRules(punkdomDir, teller.StyleRules)
	}
	...
	conversation := newInteractiveConversation(store, punkdomDir, workspace, storyID, branchID, message, runtimeCfg.InteractiveReplyTargetChars, &runtimeCfg)
...
func (s *InteractiveAppService) InteractiveTellers() ([]interactive.Teller, error) {
	cfg := s.cfg()
	if cfg == nil || cfg.PunkdomDir == "" {
		return nil, ErrNoWorkspace
	}
	return interactive.NewTellerLibrary(cfg.PunkdomDir).List()
}
...
func (s *InteractiveAppService) InteractiveTeller(id string) (interactive.Teller, error) {
	...
	return interactive.NewTellerLibrary(cfg.PunkdomDir).Get(id)
}
...
func (s *InteractiveAppService) CreateInteractiveTeller(teller interactive.Teller) (interactive.Teller, error) {
	...
	return interactive.NewTellerLibrary(cfg.PunkdomDir).Create(teller)
}
...
func (s *InteractiveAppService) UpdateInteractiveTeller(id string, teller interactive.Teller) (interactive.Teller, error) {
	...
	return interactive.NewTellerLibrary(cfg.PunkdomDir).Update(id, teller)
}
...
func (s *InteractiveAppService) DeleteInteractiveTeller(id string) error {
	...
	return interactive.NewTellerLibrary(cfg.PunkdomDir).Delete(id)
}
