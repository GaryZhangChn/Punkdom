...
func (s *ChatAppService) prepareIDEChatRuntime(req agent.ChatRequest, abortRunning bool) (ideChatRuntime, agent.ChatRequest, error) {
	...
	punkdomDir := runtime.cfg.PunkdomDir
	a.mu.Unlock()

	if layered, err := config.LoadLayered(punkdomDir, runtime.workspace); err == nil {
		...
		if len(req.StyleReferences) == 0 {
			teller := loadInteractiveTeller(punkdomDir, runtime.cfg.IDEStoryTellerID)
			if len(teller.StyleRules) > 0 {
				converted := convertTellerStyleRules(punkdomDir, teller.StyleRules)
...
func convertTellerStyleRules(punkdomDir string, rules []interactive.StyleRule) []agent.StyleRule {
	converted := make([]agent.StyleRule, 0, len(rules))
	for _, r := range rules {
		styles := resolveStyleRulePaths(punkdomDir, r.Styles)
...
func resolveStyleRulePaths(punkdomDir string, styles []string) []string {
	paths := make([]string, 0, len(styles))
	seen := make(map[string]bool, len(styles))
	for _, style := range styles {
		path := resolveStyleRulePath(punkdomDir, style)
		if path == "" || seen[path] {
			continue
		}
		seen[path] = true
...
func resolveStyleRulePath(punkdomDir string, style string) string {
	...
	if filepath.IsAbs(style) || punkdomDir == "" {
	...
	return filepath.Join(book.UserStyleDir(punkdomDir), cleanStyle)
}
