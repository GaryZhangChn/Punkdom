...
func TestInteractiveConversationBuildsHistoryAndPersistsAssistantToStory(t *testing.T) {
	...
	conversation := newInteractiveConversation(store, punkdomDir, workspace, story.ID, "", "我点燃火把", story.ReplyTargetChars, nil)
...
func TestInteractiveConversationIgnoresLegacyTellerReplyTargetChars(t *testing.T) {
	workspace := t.TempDir()
	punkdomDir := t.TempDir()
	tellerDir := filepath.Join(punkdomDir, "story-tellers")
  ...
  "slots": [
	...
	conversation := newInteractiveConversation(store, punkdomDir, workspace, story.ID, "", "我观察四周", story.ReplyTargetChars, nil)
...
func TestInteractiveConversationUsesAgentContextRecentTurns(t *testing.T) {
	...
	conversation := newInteractiveConversation(store, punkdomDir, workspace, story.ID, "", "我继续探索", story.ReplyTargetChars, cfg)
