...
func TestAppSettingsReturnsLayered(t *testing.T) {
	...
	a := &App{
		cfg:       &config.Config{Workspace: ws, PunkdomDir: punkdomDir, OpenAIModel: "x"},
		workspace: ws,
	}
	...
	if layered.Paths.UserConfig == "" || layered.Paths.WorkspaceConfig == "" || layered.Paths.PunkdomDir == "" {
...
func TestAppUpdateUserSettingsPersists(t *testing.T) {
	...
	a := &App{
		cfg:       &config.Config{Workspace: ws, PunkdomDir: punkdomDir},
		workspace: ws,
	}
	...
	out, err := config.ReadSettingsFile(filepath.Join(punkdomDir, "config.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAppUpdateWorkspaceSettingsPersists(t *testing.T) {
	...
	out, err := config.ReadSettingsFile(filepath.Join(ws, ".punkdom", "config.toml"))
	if err != nil {
		t.Fatal(err)
	}
