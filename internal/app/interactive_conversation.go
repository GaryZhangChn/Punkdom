...
func newInteractiveConversation(store *interactive.Store, punkdomDir, workspace, storyID, branchID, user string, replyTargetChars int, cfg *config.Config) *interactiveConversation {
	return &interactiveConversation{store: store, punkdomDir: punkdomDir, workspace: workspace, cfg: cfg, storyID: storyID, branchID: branchID, user: user, replyTargetChars: replyTargetChars}
}
...
func (c *interactiveConversation) teller(tellerID string) interactive.Teller {
	return loadInteractiveTeller(c.punkdomDir, tellerID)
}

func loadInteractiveTeller(punkdomDir, tellerID string) interactive.Teller {
	if punkdomDir == "" {
		return interactive.Teller{}
	}
	teller, err := interactive.NewTellerLibrary(punkdomDir).Get(tellerID)
	if err == nil {
		return teller
	}
	log.Printf("[interactive-agent] load teller failed id=%s err=%v", tellerID, err)
	fallback, fallbackErr := interactive.NewTellerLibrary(punkdomDir).Get("classic")
	if fallbackErr != nil {
...
func interactiveStateSourceSummary(title, origin string, teller interactive.Teller, loreItems, characters, worldBuilding, storyMemorySchema, snapshotState, recentTurns, userAction, narrative string) string {
	...
	if strings.TrimSpace(loreItems) != "" {
		parts = append(parts, interactiveContextSource{Source: "资料库", Title: ".punkdom/lore/items.json", Content: loreItems})
	}
	parts = append(parts,
