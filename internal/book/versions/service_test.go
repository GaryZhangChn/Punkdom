...
func TestGoGitVersionCreateDiffAndRestore(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".punkdom", "versions")); !os.IsNotExist(err) {
		t.Fatalf("should not create .punkdom/versions metadata directory, err=%v", err)
	}
	writeFile(t, dir, ".punkdom/sessions/internal.txt", "内部数据")
	writeFile(t, dir, ".punkdom/lore/items.json", "[]")
	writeFile(t, dir, ".gitignore", ".punkdom\n")
	files, err := service.commitFiles(first.Version.ID)
	if err != nil {
		t.Fatalf("commitFiles failed: %v", err)
	}
	if _, ok := files[".punkdom/sessions/internal.txt"]; ok {
		t.Fatalf("first commit should not include .punkdom file created later: %v", sortedVersionFilePaths(files))
	}
	...
	if _, ok := secondFiles[".punkdom/sessions/internal.txt"]; !ok {
		t.Fatalf("second commit should include .punkdom creative state: %v", sortedVersionFilePaths(secondFiles))
	}
	if _, ok := secondFiles[".punkdom/lore/items.json"]; !ok {
		t.Fatalf("second commit should include .punkdom lore state: %v", sortedVersionFilePaths(secondFiles))
	}
	...
	if _, err := os.Stat(filepath.Join(dir, ".punkdom", "sessions", "internal.txt")); !os.IsNotExist(err) {
		t.Fatalf("restore should remove .punkdom content absent from target version, err=%v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, ".punkdom", "lore", "items.json")); !os.IsNotExist(err) {
		t.Fatalf("restore should remove .punkdom lore content absent from target version, err=%v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, ".punkdom", "versions")); !os.IsNotExist(err) {
		t.Fatalf("restore should not create .punkdom/versions metadata directory, err=%v", err)
	}
...
func TestGoGitVersionTracksPunkdomDeletesWhenGitIgnored(t *testing.T) {
	dir := t.TempDir()
	service := NewService(dir)
	settings := DefaultAutoSettings()
	writeFile(t, dir, ".gitignore", ".punkdom\n")
	...
	if _, ok := firstFiles[".punkdom/lore/items.json"]; !ok {
		t.Fatalf("first commit should include ignored .punkdom file: %v", sortedVersionFilePaths(firstFiles))
	}

	if err := os.Remove(filepath.Join(dir, ".punkdom", "lore", "items.json")); err != nil {
		t.Fatal(err)
	}
	status, err := service.Status(settings)
	if err != nil {
	...
	assertChange(t, status.Changes, ".punkdom/lore/items.json", "deleted")
	...
	if _, ok := secondFiles[".punkdom/lore/items.json"]; ok {
		t.Fatalf("second commit should record ignored .punkdom deletion: %v", sortedVersionFilePaths(secondFiles))
	}
}
...
func TestGoGitVersionExcludesRunLedgers(t *testing.T) {
	...
	settings := DefaultAutoSettings()
	writeFile(t, dir, "chapters/ch0001.md", "第一版")
	writeFile(t, dir, ".punkdom/runs/run-1.jsonl", `{"type":"run_created"}`)
	...
	if _, ok := files[".punkdom/runs/run-1.jsonl"]; ok {
		t.Fatalf("run ledger should not be committed: %v", sortedVersionFilePaths(files))
	}
	if _, err := os.Stat(filepath.Join(dir, ".punkdom", "runs", "run-1.jsonl")); err != nil {
		t.Fatalf("run ledger should remain in workspace: %v", err)
	}

	writeFile(t, dir, ".punkdom/runs/run-2.jsonl", `{"type":"run_finished"}`)
	status, err := service.Status(settings)
	if err != nil {
		t.Fatalf("Status failed: %v", err)
	}
