...
func TestStoreSeparatesUserAndWorkspaceTasks(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(workspace, ".punkdom"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(workspace, ".punkdom", "automations", "tasks.json")); err != nil {
...
func TestStoreAppendRunUpdatesExistingRun(t *testing.T) {
	...
	store := NewStore(filepath.Join(root, "punkdom"), workspace)
