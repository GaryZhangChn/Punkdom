...
func NewStore(userPunkdomDir, workspace string) *Store {
	return &Store{userDir: userPunkdomDir, workspace: workspace}
}
...
func (s *Store) pathForScope(scope string) (string, error) {
	...
	case ScopeUser:
		if strings.TrimSpace(s.userDir) == "" {
			return "", fmt.Errorf("user punkdom dir is required")
		}
	...
	case ScopeWorkspace:
		...
		return filepath.Join(s.workspace, ".punkdom", "automations", "tasks.json"), nil
	default:
