...
var catalogZH = map[string]string{
	...
	"api.books.punkdomDirMissing":                "Punkdom 数据目录未配置",
	...
	"api.interactive.tellerInstructionEmpty":  "叙事编排编辑指令不能为空",
	"api.lore.instructionEmpty":               "资料库编辑指令不能为空",
	"api.styles.failed":                       "获取风格参考失败: {{detail}}",
...
var catalogEN = map[string]string{
	...
	"api.books.punkdomDirMissing":                "Punkdom data directory is not configured.",
	...
	"api.styles.failed":                       "Failed to load style references: {{detail}}",
