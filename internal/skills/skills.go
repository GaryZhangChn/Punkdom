...
// NewDirectories returns the canonical skill search path for Punkdom.
func NewDirectories(builtinDir, punkdomDir, workspace string) []Directory {
	...
	if path := normalizePath(filepath.Join(punkdomDir, "skills")); punkdomDir != "" && path != "" {
		dirs = append(dirs, Directory{Scope: ScopeUser, Path: path, Writable: true})
	}
	if path := normalizePath(filepath.Join(workspace, ".punkdom", "skills")); workspace != "" && path != "" {
		dirs = append(dirs, Directory{Scope: ScopeWorkspace, Path: path, Writable: true})
	}
	return dirs
}

// Backend adapts multiple Punkdom skill directories to Eino's skill.Backend.
type Backend struct {
	dirs      []Directory
	agentKind string
