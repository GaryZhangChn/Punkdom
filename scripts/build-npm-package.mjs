...
const platforms = [
  { key: "darwin-arm64", goos: "darwin", goarch: "arm64", exe: "punkdom" },
  { key: "darwin-x64", goos: "darwin", goarch: "amd64", exe: "punkdom" },
  { key: "linux-arm64", goos: "linux", goarch: "arm64", exe: "punkdom" },
  { key: "linux-x64", goos: "linux", goarch: "amd64", exe: "punkdom" },
  { key: "win32-x64", goos: "windows", goarch: "amd64", exe: "punkdom.exe" },
];
...
function main() {
  ...
  for (const name of ["README.md", "CHANGELOG.md", "LICENSE", "NOTICE", "config.toml"]) {
  ...
  cpFileIfExists("NOTICE");

  console.log("==> 交叉编译 Punkdom");
  for (const target of platforms) {
    const outDir = join(packageDir, "vendor", target.key);
    mkdirSync(outDir, { recursive: true });
    const out = join(outDir, target.exe);
    run("go", ["build", "-trimpath", "-ldflags", `-s -w -X punkdom/internal/buildinfo.Version=${packageVersion}`, "-o", out, "./cmd/punkdom"], {
      ...process.env,
      CGO_ENABLED: "0",
      GOOS: target.goos,
  ...
  chmodSync(join(packageDir, "bin", "punkdom.js"), 0o755);
